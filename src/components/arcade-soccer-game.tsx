...
import {
  ...
  normalizeOfflineSettings as normalizeAnonymousSettings,
  runAnonymousSetupSelfTests,
  validateTeamSetup,
  type BodyPresetId,
  type OfflineSettings as AnonymousOfflineSettings,
  type PlayerLine,
} from "@/lib/anonymous-team-setup";

type OfflineSettings = AnonymousOfflineSettings;
...
type LocomotionState = "idle" | "walk" | "jog" | "run" | "sprint" | "acceleration" | "deceleration"
  | "sharp-turn" | "lateral-shuffle" | "backpedal" | "dribble-jog" | "dribble-sprint"
  | "pass-approach" | "shot-approach" | "recovery";
type BallTouchBodyPart = "foot" | "head" | "chest" | "knee" | "other";
type BallTouchAction = "pass" | "clearance" | "shot" | "deflection" | "control" | "tackle" | "throwIn";

type BallTouchRecord = {
  teamId: TeamId;
  playerId: PlayerId;
...
const BALL_LEAVE_GROUNDED_THRESHOLD = BALL_RADIUS + 0.11;
const SHARED_BALL_PHYSICS = {
  radius: BALL_RADIUS,
...
const STAMINA_RECOVERY_RATE = 0.22;
const STAMINA_RECOVERY_THRESHOLD = 0.22;
const SHOT_INPUT_LOCK_SECONDS = 0.32;
const DEFENSIVE_PRESSING_DISTANCE = 18;
const DEFENSIVE_REACTION_DELAY = 0.08;
const DEFENSIVE_MARKING_DISTANCE = 3.6;
const DEFENSIVE_TACKLE_AGGRESSION = 0.68;
const DEFENSIVE_INTERCEPTION_RANGE = 5.2;
const DEFENSIVE_RECOVERY_SPEED = 1.08;
const TOUCHLINE_RUNOFF = 6.1;
const GOAL_LINE_RUNOFF = 8.2;
...
function fitDirectionalShadowCameraToPitch(light: THREE.DirectionalLight) {
...
const LOCOMOTION_REFERENCE_SPEEDS = {
  walk: 1.6,
  jog: 3.8,
  run: 6.8,
  sprint: 9.5,
...
function fixedFormationSettings(settings: unknown): OfflineSettings {
  const normalized = normalizeAnonymousSettings(settings);
  return {
    ...normalized,
...
let activeOfflineSettings: OfflineSettings = fixedFormationSettings(ANONYMOUS_DEFAULT_SETTINGS);
...
function loadOfflineSettings() {
  if (typeof window === "undefined") return fixedFormationSettings(ANONYMOUS_DEFAULT_SETTINGS);
  try {
    ...
    return fixedFormationSettings(ANONYMOUS_DEFAULT_SETTINGS);
  }
}
...
function createMinimapSnapshot(active: MatchRuntime): MinimapSnapshot {
  return {
...
function syncBallVisual(active: MatchRuntime) {
  if (
    ...
    const alpha = clamp(active.ballPhysicsAccumulator / BALL_PHYSICS_STEP, 0, 1);
    active.ballVisualPosition.copy(active.previousBallPhysicsPos).lerp(active.ballPos, alpha);
  } else {
    active.ballVisualPosition.copy(active.ballPos);
  }
  active.ball.position.copy(active.ballVisualPosition);
  active.ballShadow.position.set(active.ballVisualPosition.x, BLOB_SHADOW_Y, active.ballVisualPosition.z);
  const height = Math.max(0, active.ballVisualPosition.y - BALL_RADIUS);
...
function recordBallTouch(
  ...
  active.lastTouchRecord = touch;
  active.renderer.domElement.dataset.lastTouchRecord = JSON.stringify(touch);
}

function deliberateFootBackPassRestriction(keeper: PlayerBody, active: MatchRuntime) {
  const touch = active.lastTouchRecord;
  if (!touch) return false;
  return touch.teamId === keeper.team
    && touch.playerId !== keeper.id
    && touch.bodyPart === "foot"
    && touch.intentional
    && touch.intendedReceiverId === keeper.id
    && (touch.action === "pass" || touch.action === "clearance");
}

function keeperMayUseHands(keeper: PlayerBody, active: MatchRuntime, margin = 0) {
  return keeper.role === "keeper"
    && pointInsideOwnPenaltyArea(keeper.team, active.half, active.ballPos, margin)
    && !deliberateFootBackPassRestriction(keeper, active);
}
...
function prepareGoalKickSetup(active: MatchRuntime, team: TeamId) {
  ...
  active.manualGoalKickTarget = null;
  return keeper;
}
...
function syncRuntimeDiagnostics(active: MatchRuntime) {
  ...
  canvas.dataset.ballOwnerId = active.ballOwnerId ?? "";
  ...
  canvas.dataset.anonymousPlayers = JSON.stringify(active.players.map((player) => ({
    id: player.id,
    team: player.team,
    slot: player.formationSlot,
    number: player.number,
    bodyPresetId: player.bodyPresetId,
  })));
  canvas.dataset.visiblePlayerNameLabels = "0";
  canvas.dataset.landingMarkerShape = "yellow-x";
  canvas.dataset.crowdVolume = activeOfflineSettings.crowdVolume.toFixed(2);
  canvas.dataset.commentaryVolume = activeOfflineSettings.commentaryVolume.toFixed(2);
...
function animationParts(mesh: THREE.Group): PlayerAnimationParts {
  return {
    ...
    pelvis: mesh.getObjectByName("pelvis-root") ?? null,
    spineLower: mesh.getObjectByName("spine-lower") ?? null,
    spineUpper: mesh.getObjectByName("spine-upper") ?? null,
    chest: mesh.getObjectByName("chest-root") ?? null,
    neckBase: mesh.getObjectByName("neck-base") ?? null,
    neckTop: mesh.getObjectByName("neck-top") ?? null,
    head: mesh.getObjectByName("head-root") ?? null,
    headContact: mesh.getObjectByName("head-contact") ?? null,
    leftClavicle: mesh.getObjectByName("left-clavicle") ?? null,
    rightClavicle: mesh.getObjectByName("right-clavicle") ?? null,
    ...
    leftFoot: mesh.getObjectByName("left-foot") ?? null,
    rightFoot: mesh.getObjectByName("right-foot") ?? null,
    ...
    leftHand: mesh.getObjectByName("left-hand") ?? null,
    rightHand: mesh.getObjectByName("right-hand") ?? null,
  };
}

function attachDevelopmentRigDebug(mesh: THREE.Group) {
  if (process.env.NODE_ENV === "production" || typeof window === "undefined") return;
  if (!new URLSearchParams(window.location.search).has("rigDebug")) return;
  [
    "pelvis-root", "spine-lower", "spine-upper", "chest-root", "neck-base", "neck-top", "head-root",
    "left-clavicle", "right-clavicle", "left-arm", "right-arm", "left-elbow", "right-elbow",
    "left-leg", "right-leg", "left-knee", "right-knee", "left-foot", "right-foot",
  ].forEach((name) => {
    const joint = mesh.getObjectByName(name);
    if (!joint) return;
    const axes = new THREE.AxesHelper(0.34);
    axes.name = `rig-debug-${name}`;
    axes.renderOrder = 80;
    joint.add(axes);
  });
}
...
function createPlayer(id: string, team: TeamId, role: PlayerRole, line: PlayerLine, x: number, z: number, number: number, formationSlot: string, controlledBy?: "p1") {
  const configuredTeam = team === "home" ? activeOfflineSettings.userTeam : activeOfflineSettings.aiTeam;
  ...
  const possessionLabel = mesh.getObjectByName("player-possession-label") ?? null;
  return {
    id,
    team,
    role,
    ...
    bodyPresetId,
    preferredFoot: configuredPlayer?.preferredFoot ?? "right",
    ...
    diveSide: 0,
    headerTimer: 0,
    previousHeadWorldPosition: new THREE.Vector3(x, 2.2, z),
    headerSampleFrame: -1,
    firstTouchTimer: 0,
    firstTouchType: null,
    ...
    animationLastPos: new THREE.Vector3(x, 0, z),
...
function formationPlayers(half: 1 | 2) {
  ...
  (["home", "away"] as TeamId[]).forEach((team) => {
    ...
    const teamSetup = team === "home" ? activeOfflineSettings.userTeam : activeOfflineSettings.aiTeam;
    const template = ANONYMOUS_FORMATIONS[teamSetup.formationId];
    let fieldIndex = 1;
...
export function ArcadeSoccerGame() {
  ...
  const [offlineSettings, setOfflineSettings] = useState<OfflineSettings>(() => fixedFormationSettings(ANONYMOUS_DEFAULT_SETTINGS));
  const [draftSettings, setDraftSettings] = useState<OfflineSettings>(() => fixedFormationSettings(ANONYMOUS_DEFAULT_SETTINGS));
  ...
  const [cameraModeUi, setCameraModeUi] = useState<"broadcast" | "first-person">("broadcast");
  ...
  const toggleCameraMode = useCallback(() => {
    ...
    if (!active) return;
    active.cameraMode = active.cameraMode === "broadcast" ? "first-person" : "broadcast";
    active.renderer.domElement.dataset.cameraMode = active.cameraMode;
    setCameraModeUi(active.cameraMode);
  }, []);

  useEffect(() => {
    const savedSettings = loadOfflineSettings();
    activeOfflineSettings = savedSettings;
    setOfflineSettings(savedSettings);
  ...
  const closeSettings = useCallback(() => {
    const active = sceneRef.current;
    setMinimapSnapshot(active && offlineSettings.minimapEnabled ? createMinimapSnapshot(active) : null);
    settingsOpenRef.current = false;
    setSettingsOpen(false);
  }, [offlineSettings.minimapEnabled]);

  const commitSettings = useCallback(() => {
    const next = fixedFormationSettings(draftSettings);
    ...
    if (active && active.state !== "playing") {
      ...
      setMinimapSnapshot(next.minimapEnabled ? createMinimapSnapshot(active) : null);
    } else if (active) {
      scoreUiRef.current = { ...active.score };
      setScore({ ...active.score });
      setMinimapSnapshot(next.minimapEnabled ? createMinimapSnapshot(active) : null);
  ...
  const requestGameFullscreen = useCallback(async () => {
    if (typeof document === "undefined" || detectAppleMobile()) return;
  ...
  const sendMobileKey = useCallback((code: string, pressed: boolean) => {
  ...
  const resetPositions = useCallback((servingTeam: TeamId = "home") => {
    ...
    active.players.forEach((player) => {
      ...
      player.previousHorizontalSpeed = 0;
      ...
      player.animationLastPos.copy(player.home);
    ...
    active.ballOwnerId = null;
    clearPassIntent(active, "reset");
    active.pendingReceiveAction = null;
    ...
    active.pendingKickContact = null;
    active.lastShotTap = 0;
    active.lastClockAdvanceTime = performance.now();
    active.shotCharge = 0;
    active.shotChargingPlayerId = null;
    active.shotInputLockTimer = 0;
    active.passCharge = 0;
    active.passChargingPlayerId = null;
    active.passCommitTimer = 0;
    active.trajectoryDiagnosticActive = false;
    active.loftCharge = 0;
  ...
  const startMatch = useCallback(() => {
    ...
    if (showTouchControls) {
      setP1AutopilotMode(active, false);
      syncP1AiUi(false);
    }
    setMinimapSnapshot(activeOfflineSettings.minimapEnabled ? createMinimapSnapshot(active) : null);
    setPhaseUi(active.phase);
  ...
  useEffect(() => {
    ...
    const update = () => setShowTouchControls(forcedTouch || detectAppleMobile() || (coarseOnly.matches && !fineInput.matches));
    update();
  ...
  useEffect(() => {
    ...
    const resolveCameraFov = (width: number) => {
      const forcedTouch = new URLSearchParams(window.location.search).get("touch") === "1";
      const coarseTouch = window.matchMedia("(any-pointer: coarse) and (any-hover: none)").matches;
      return forcedTouch || coarseTouch || width < 700 ? 38 : 30;
    };
    const camera = new THREE.PerspectiveCamera(resolveCameraFov(mount.clientWidth), mount.clientWidth / mount.clientHeight, 0.1, 260);
    ...
    const applyRendererSize = () => {
      ...
      camera.fov = resolveCameraFov(width);
      camera.updateProjectionMatrix();
    ...
    const onFieldPointerDown = (event: PointerEvent) => {
      ...
      if (active.phase === "goal-kick" && active.restartTeam === "home") {
        if (raycaster.ray.intersectPlane(fieldPlane, kickPoint)) {
          active.manualGoalKickTarget = kickPoint.clone().set(
            clamp(kickPoint.x, -FIELD_W / 2 + 1.2, FIELD_W / 2 - 1.2),
            BALL_RADIUS,
            clamp(kickPoint.z, -FIELD_L / 2 + 1.2, FIELD_L / 2 - 1.2),
          );
          active.manualGoalKickReceiverId = null;
          active.manualRestartTargetId = null;
          active.manualRestartTargetZone = null;
    ...
    // Keep every pitch corner in front of the shadow camera. The previous low
    // light position left the far touchline behind the light and clipped casts.
    sun.position.set(24, 100, 30);
    sun.castShadow = true;
    sun.shadow.mapSize.set(2048, 2048);
    fitDirectionalShadowCameraToPitch(sun);
    ...
    const runtime: MatchRuntime = {
      ...
      ballVisualPosition: new THREE.Vector3(0, BALL_RADIUS, 0),
    ...
    runtime.renderer.domElement.dataset.cameraMode = runtime.cameraMode;
    ...
    const requestedAnonymousSetupTests = clamp(
      Number(new URLSearchParams(window.location.search).get("anonymousSetupTest") ?? "0"),
      0,
      1000,
    );
    const anonymousSetupResults = runAnonymousSetupSelfTests(requestedAnonymousSetupTests);
    runtime.renderer.domElement.dataset.anonymousSetupTestsRequested = String(requestedAnonymousSetupTests);
    runtime.renderer.domElement.dataset.anonymousSetupTeamsGenerated = String(anonymousSetupResults.generated);
    runtime.renderer.domElement.dataset.anonymousSetupTestsFailed = String(anonymousSetupResults.failures.length);
    runtime.renderer.domElement.dataset.anonymousSetupTestFailures = JSON.stringify(anonymousSetupResults.failures.slice(0, 20));
    ...
    const slowMotionScale = clamp(Number(queryParameters.get("slowMotion") ?? "1"), 0.1, 1);
    const anonymousPossessionTest = queryParameters.has("anonymousPossessionTest");
    const modelInspectView = queryParameters.get("modelInspect");
    let anonymousPossessionTestApplied = false;
    runtime.renderer.domElement.dataset.slowMotionScale = slowMotionScale.toFixed(2);
    const requestedGoalKickTests = clamp(
      Number(
        queryParameters.get("goalKickTest")
        ?? queryParameters.get("manualGoalKickTest")
        ?? "0",
      ),
      0,
      10,
    ...
    const requestedPassIntentTests = clamp(Number(new URLSearchParams(window.location.search).get("passIntentTest") ?? "0"), 0, 600);
    ...
    let pendingPassIntentCheck: {
      category: "centre-back" | "fullback" | "defensive-midfielder" | "goalkeeper-foot" | "pressured-clearance" | "attacking";
      receiverId: string;
      target: THREE.Vector3;
    ...
    const requestedOneTouchTests = clamp(Number(queryParameters.get("oneTouchTest") ?? "0"), 0, 60);
    let remainingOneTouchTests = requestedOneTouchTests;
    let nextOneTouchTestAt = performance.now() + 900;
    runtime.renderer.domElement.dataset.oneTouchTestsRequested = String(requestedOneTouchTests);
    runtime.renderer.domElement.dataset.oneTouchTestsPassed = "0";
    runtime.renderer.domElement.dataset.oneTouchTestsFailed = "0";
    const requestedTrajectoryTests = clamp(Number(queryParameters.get("trajectoryTest") ?? "0"), 0, 24);
    let remainingTrajectoryTests = requestedTrajectoryTests;
    let nextTrajectoryTestAt = performance.now() + 900;
    let pendingTrajectoryTest: {
      index: number;
      style: KickStyle;
      predicted: THREE.Vector3;
      closestError: number;
      closestHorizontalError: number;
      closestVerticalError: number;
      closestPoint: THREE.Vector3;
      previewLaunchPosition: unknown;
      previewLaunchVelocity: unknown;
      previewLaunchCurve: unknown;
      at: number;
    } | null = null;
    runtime.renderer.domElement.dataset.trajectoryTestsRequested = String(requestedTrajectoryTests);
    runtime.renderer.domElement.dataset.trajectoryTestsPassed = "0";
    runtime.renderer.domElement.dataset.trajectoryTestsFailed = "0";
    ...
    const automatedScenarioActive = Boolean(
      ...
      || requestedOneTouchTests
      || requestedTrajectoryTests
    ...
    const runMatchTick = (now = performance.now()) => {
      ...
      active.matchUpdatesThisFrame = 0;
      const dt = Math.min(Math.max((now - active.lastMatchUpdate) / 1000, 0), 0.033) * slowMotionScale;
      active.lastMatchUpdate = now;
      if (dt <= 0) return;
      if (requestedPassIntentTests > 0 && (remainingPassIntentTests > 0 || pendingPassIntentCheck)) {
        active.gameClock = 0;
        active.lastClockAdvanceTime = now;
      }
      ...
      try {
        ...
        if (anonymousPossessionTest && !anonymousPossessionTestApplied && active.phase === "open") {
          const testPlayer = active.players.find((player) => player.team === "home" && player.role !== "keeper") ?? null;
          if (testPlayer) {
            releasePossession(active, "loose");
            testPlayer.pos.set(0, 0, 4);
            testPlayer.vel.set(0, 0, 0);
            active.ballPos.copy(testPlayer.pos).add(facingDirection(testPlayer).multiplyScalar(0.58)).setY(BALL_RADIUS);
            active.ballVel.set(0, 0, 0);
            takePossession(testPlayer, active);
            anonymousPossessionTestApplied = true;
            active.renderer.domElement.dataset.anonymousPossessionTestApplied = "true";
          }
        }
        if (
        ...
        if (remainingBlockedPassTests > 0 && active.phase === "open" && now >= nextBlockedPassTestAt) {
          ...
          const actor = active.players.find((player) => player.team === team && player.line === "defender" && player.role !== "keeper" && !player.sentOff) ?? null;
        ...
        if (pendingKeeperBuildupCheck && now >= pendingKeeperBuildupCheck.at) {
          ...
          const legalFootControl = received
            && pendingKeeperBuildupCheck.handledWithFoot
            && !pendingKeeperBuildupCheck.usedHands;
          ...
          previousResults.push({
            ...
            received,
            usedHands: pendingKeeperBuildupCheck.usedHands,
            handledWithFoot: pendingKeeperBuildupCheck.handledWithFoot,
            legalFootControl,
          ...
          const resultKey = legalFootControl ? "keeperBuildupTestsPassed" : "keeperBuildupTestsFailed";
          active.renderer.domElement.dataset[resultKey] = String(Number(active.renderer.domElement.dataset[resultKey] ?? "0") + 1);
        ...
        if (pendingHeaderTestCheck && now >= pendingHeaderTestCheck.at) {
          ...
          const results = active.renderer.domElement.dataset.headerTestResults
            ...
            : [];
          results.push({
            kind: pendingHeaderTestCheck.kind,
            expected: pendingHeaderTestCheck.expected,
            contacted,
            reason: active.renderer.domElement.dataset.headerRejectReason ?? "",
          });
        ...
        if (remainingOneTouchTests > 0 && active.phase === "open" && now >= nextOneTouchTestAt) {
          const testIndex = requestedOneTouchTests - remainingOneTouchTests;
          const homeOutfield = active.players.filter((player) => player.team === "home" && player.role !== "keeper" && !player.sentOff);
          const passer = homeOutfield.find((player) => player.line === "midfielder") ?? null;
          const receiver = homeOutfield.find((player) => player.id !== passer?.id && player.line === "midfielder") ?? null;
          const nextReceiver = homeOutfield.find((player) => player.id !== passer?.id && player.id !== receiver?.id && player.line === "forward") ?? null;
          if (passer && receiver && nextReceiver) {
            active.players.forEach((player, index) => {
              if (player.id === passer.id || player.id === receiver.id || player.id === nextReceiver.id || player.role === "keeper") return;
              player.pos.set(player.team === "home" ? -FIELD_W / 2 + 5 : FIELD_W / 2 - 5, 0, -FIELD_L / 2 + 6 + (index % 9) * 8);
              player.vel.set(0, 0, 0);
              player.mesh.position.copy(player.pos);
            });
            passer.pos.set(-2, 0, 10);
            receiver.pos.set(0, 0, -4);
            nextReceiver.pos.set(testIndex % 2 === 0 ? -9 : 9, 0, -23);
            [passer, receiver, nextReceiver].forEach((player) => {
              player.vel.set(0, 0, 0);
              player.mesh.position.copy(player.pos);
              player.actionCooldown = 0;
              player.kickTimer = 0;
            ...
            receiver.heading = headingFromDirection(nextReceiver.pos.clone().sub(receiver.pos).setY(0));
            receiver.mesh.rotation.y = receiver.heading;
            setControlledPlayer(active, passer, "p1");
            releasePossession(active, "kicked");
            active.ballPos.copy(receiver.pos).setY(BALL_RADIUS);
            active.ballVel.copy(receiver.pos).sub(passer.pos).setY(0).normalize().multiplyScalar(16);
            active.ballState = "kicked";
            active.lastTouchTeam = "home";
            active.intendedReceiverId = receiver.id;
            active.manualPassReceiverId = receiver.id;
            active.passIntent = {
              passerId: passer.id,
              receiverId: receiver.id,
              receiverTeamId: "home",
              kickTime: performance.now() - 250,
              passType: "short",
              state: "track",
              initialDirection: receiver.pos.clone().sub(passer.pos).setY(0).normalize(),
              target: receiver.pos.clone().setY(BALL_RADIUS),
              predictedReceptionPoint: receiver.pos.clone().setY(BALL_RADIUS),
              predictedArrivalTime: 0.1,
              initialPower: 16,
              elapsed: 0.25,
              intoSpace: false,
            };
            active.cooldown = 0;
            const buffered = bufferUserPassBeforeReception(active, new Set<string>(), testIndex % 3 === 0 ? "through" : "short");
            const claimed = takePossession(receiver, active);
            const queuedOneTouch = active.pendingKickContact?.playerId === receiver.id
              && active.pendingKickContact.receiverId === nextReceiver.id;
            const executed = active.renderer.domElement.dataset.lastReceiveAction === "oneTouchPass"
              && active.renderer.domElement.dataset.lastReceiveActionReceiver === receiver.id
              && (queuedOneTouch || (active.ballState === "kicked" && active.intendedReceiverId === nextReceiver.id));
            const passed = buffered && claimed && executed;
            const key = passed ? "oneTouchTestsPassed" : "oneTouchTestsFailed";
            active.renderer.domElement.dataset[key] = String(Number(active.renderer.domElement.dataset[key] ?? "0") + 1);
            const results = active.renderer.domElement.dataset.oneTouchTestResults
              ? JSON.parse(active.renderer.domElement.dataset.oneTouchTestResults) as unknown[]
              : [];
            results.push({ index: testIndex, buffered, claimed, executed, style: testIndex % 3 === 0 ? "through" : "short" });
            active.renderer.domElement.dataset.oneTouchTestResults = JSON.stringify(results);
            remainingOneTouchTests -= 1;
            nextOneTouchTestAt = now + 135;
          }
        }
        if (pendingTrajectoryTest) {
          const continuousError = sweptPointDistance(
            pendingTrajectoryTest.predicted,
            active.previousBallPhysicsPos,
            active.ballPos,
          ).distance;
          const predictedHorizontal = pendingTrajectoryTest.predicted.clone().setY(0);
          const previousHorizontal = active.previousBallPhysicsPos.clone().setY(0);
          const currentHorizontal = active.ballPos.clone().setY(0);
          const continuousHorizontalError = sweptPointDistance(
            predictedHorizontal,
            previousHorizontal,
            currentHorizontal,
          ).distance;
          if (continuousError < pendingTrajectoryTest.closestError) {
            const segment = active.ballPos.clone().sub(active.previousBallPhysicsPos);
            const segmentLengthSq = segment.lengthSq();
            const projection = segmentLengthSq > 0.000001
              ? clamp(pendingTrajectoryTest.predicted.clone().sub(active.previousBallPhysicsPos).dot(segment) / segmentLengthSq, 0, 1)
              : 0;
            pendingTrajectoryTest.closestPoint.copy(active.previousBallPhysicsPos).addScaledVector(segment, projection);
            pendingTrajectoryTest.closestError = continuousError;
          }
          pendingTrajectoryTest.closestHorizontalError = Math.min(
            pendingTrajectoryTest.closestHorizontalError,
            continuousHorizontalError,
          );
          pendingTrajectoryTest.closestVerticalError = Math.min(
            pendingTrajectoryTest.closestVerticalError,
            Math.abs(active.ballPos.y - pendingTrajectoryTest.predicted.y),
          );
          if (now >= pendingTrajectoryTest.at) {
            const tolerance = pendingTrajectoryTest.style === "long"
              ? 2
              : pendingTrajectoryTest.style === "shot" || pendingTrajectoryTest.style === "finesse"
                ? 2
                : 1.5;
            const passed = pendingTrajectoryTest.closestError <= tolerance;
            const resultKey = passed ? "trajectoryTestsPassed" : "trajectoryTestsFailed";
            active.renderer.domElement.dataset[resultKey] = String(Number(active.renderer.domElement.dataset[resultKey] ?? "0") + 1);
            const results = active.renderer.domElement.dataset.trajectoryTestResults
              ? JSON.parse(active.renderer.domElement.dataset.trajectoryTestResults) as unknown[]
              : [];
            results.push({
              index: pendingTrajectoryTest.index,
              style: pendingTrajectoryTest.style,
              error: Number(pendingTrajectoryTest.closestError.toFixed(3)),
              horizontalError: Number(pendingTrajectoryTest.closestHorizontalError.toFixed(3)),
              verticalError: Number(pendingTrajectoryTest.closestVerticalError.toFixed(3)),
              predicted: pendingTrajectoryTest.predicted.toArray().map((value) => Number(value.toFixed(3))),
              releaseEndpoint: JSON.parse(active.renderer.domElement.dataset.kickReleaseTrajectoryEndpoint ?? "null") as unknown,
              releasePosition: JSON.parse(active.renderer.domElement.dataset.kickReleasePosition ?? "null") as unknown,
              releaseVelocity: JSON.parse(active.renderer.domElement.dataset.kickReleaseVelocity ?? "null") as unknown,
              releaseCurve: JSON.parse(active.renderer.domElement.dataset.kickReleaseCurve ?? "null") as unknown,
              closestPoint: pendingTrajectoryTest.closestPoint.toArray().map((value) => Number(value.toFixed(3))),
              previewLaunchPosition: pendingTrajectoryTest.previewLaunchPosition,
              previewLaunchVelocity: pendingTrajectoryTest.previewLaunchVelocity,
              previewLaunchCurve: pendingTrajectoryTest.previewLaunchCurve,
              tolerance,
              passed,
            });
            active.renderer.domElement.dataset.trajectoryTestResults = JSON.stringify(results);
            pendingTrajectoryTest = null;
            active.trajectoryDiagnosticActive = false;
            nextTrajectoryTestAt = now + 120;
          }
        }
        if (
          remainingTrajectoryTests > 0
          && !pendingTrajectoryTest
          && active.phase === "open"
          && now >= nextTrajectoryTestAt
        ) {
          const testIndex = requestedTrajectoryTests - remainingTrajectoryTests;
          const style = (["short", "long", "shot", "finesse"] as KickStyle[])[testIndex % 4];
          const team: TeamId = Math.floor(testIndex / 4) % 2 === 0 ? "home" : "away";
          const actor = active.players.find((player) => player.team === team && player.role !== "keeper" && !player.sentOff) ?? null;
          if (actor) {
            const attackSign = Math.sign(attackingGoalZ(team, active.half));
            const distanceBand = Math.floor(testIndex / 8) % 3;
            const distances = style === "short" ? [12, 27, 43] : style === "long" ? [25, 44, 64] : [22, 31, 38];
            const distance = distances[distanceBand];
            const lateral = (testIndex % 3 - 1) * (style === "finesse" ? 8 : 3.5);
            actor.pos.set(style === "finesse" ? -attackSign * 14 : 0, 0, -attackSign * 18);
            actor.vel.set(0, 0, 0);
            actor.heading = headingFromDirection(new THREE.Vector3(0, 0, attackSign));
            actor.mesh.rotation.y = actor.heading;
            actor.mesh.position.copy(actor.pos);
            active.players.forEach((player, index) => {
              if (player.id === actor.id || player.role === "keeper") return;
              player.pos.set(player.team === team ? -FIELD_W / 2 + 3 : FIELD_W / 2 - 3, 0, -FIELD_L / 2 + 5 + (index % 10) * 8);
              player.vel.set(0, 0, 0);
              player.mesh.position.copy(player.pos);
            });
            const target = style === "shot" || style === "finesse"
              ? new THREE.Vector3(lateral, 1.25, attackingGoalZ(team, active.half))
              : actor.pos.clone().add(new THREE.Vector3(lateral, 0, attackSign * distance)).setY(BALL_RADIUS);
            const receiver = isPassKickStyle(style)
              ? active.players.find((player) => player.team === team && player.id !== actor.id && player.role !== "keeper" && !player.sentOff) ?? null
              : null;
            if (receiver) {
              receiver.pos.copy(target).setY(0);
              receiver.vel.set(0, 0, 0);
              receiver.mesh.position.copy(receiver.pos);
            }
            releasePossession(active, "loose");
            active.ballPos.copy(actor.pos).addScaledVector(facingDirection(actor), 0.72).setY(BALL_RADIUS);
            active.ballVel.set(0, 0, 0);
            active.ballCurve.set(0, 0, 0);
            actor.actionCooldown = 0;
            actor.kickTimer = 0;
            ...
            takePossession(actor, active);
            active.cooldown = 0;
            const charge = [0.24, 0.58, 0.92][distanceBand];
            updateKickTrajectoryPreview(active, actor, target, style, charge, new THREE.Vector3(), null, receiver);
            const predicted = (style === "long" ? active.kickLandingZone : active.kickPreviewEndpoint).position.clone();
            const kicked = kickTowardPoint(actor, target, active, style, receiver ?? undefined, charge, true);
            if (kicked) {
              active.trajectoryDiagnosticActive = true;
              pendingTrajectoryTest = {
                index: testIndex,
                style,
                predicted,
                closestError: active.ballPos.distanceTo(predicted),
                closestHorizontalError: active.ballPos.clone().setY(0).distanceTo(predicted.clone().setY(0)),
                closestVerticalError: Math.abs(active.ballPos.y - predicted.y),
                closestPoint: active.ballPos.clone(),
                previewLaunchPosition: JSON.parse(active.renderer.domElement.dataset.previewLaunchPosition ?? "null") as unknown,
                previewLaunchVelocity: JSON.parse(active.renderer.domElement.dataset.previewLaunchVelocity ?? "null") as unknown,
                previewLaunchCurve: JSON.parse(active.renderer.domElement.dataset.previewLaunchCurve ?? "null") as unknown,
                at: now + (style === "shot" || style === "finesse" || style === "chip"
                  ? 4400
                  : style === "long"
                    ? 3600
                    : 2600),
              ...
              active.trajectoryDiagnosticActive = false;
              active.renderer.domElement.dataset.trajectoryTestsFailed = String(Number(active.renderer.domElement.dataset.trajectoryTestsFailed ?? "0") + 1);
              nextTrajectoryTestAt = now + 120;
            }
            remainingTrajectoryTests -= 1;
            active.renderer.domElement.dataset.trajectoryTestsRemaining = String(remainingTrajectoryTests);
          }
        }
        ...
        if (pendingPassIntentCheck && (
          ...
          pendingPassIntentCheck.received = true;
          pendingPassIntentCheck.actualReceiver = pendingPassIntentCheck.receiverId;
        ...
        if (pendingPassIntentCheck) {
          const trackedReceiver = active.players.find((player) => player.id === pendingPassIntentCheck?.receiverId) ?? null;
          if (trackedReceiver) {
            pendingPassIntentCheck.closestApproachDistance = Math.min(
              pendingPassIntentCheck.closestApproachDistance,
              trackedReceiver.pos.distanceTo(active.ballPos),
            );
          }
        }
        if (pendingPassIntentCheck && now >= pendingPassIntentCheck.at) {
          ...
          const received = pendingPassIntentCheck.received
            || active.ballOwnerId === pendingPassIntentCheck.receiverId
            || active.renderer.domElement.dataset.lastReceived === pendingPassIntentCheck.receiverId;
          ...
          const closestApproachDistance = pendingPassIntentCheck.closestApproachDistance;
          ...
          previousResults.push({
            ...
            category: pendingPassIntentCheck.category,
            style: pendingPassIntentCheck.style,
            passed,
            received,
            intendedReceiver: pendingPassIntentCheck.receiverId,
            actualReceiver: pendingPassIntentCheck.actualReceiver
              ?? active.ballOwnerId
              ?? active.renderer.domElement.dataset.lastReceived
              ?? "",
        ...
        if (
          ...
          const matrixIndex = requestedPassIntentTests >= 500 ? testIndex : testIndex % 500;
          const category = matrixIndex < 100
            ? "centre-back"
            : matrixIndex < 200
              ? "fullback"
              : matrixIndex < 300
          ...
          const sameTeam = active.players.filter((player) => player.team === passingTeam && !player.sentOff);
          const passerPool = category === "centre-back"
            ? sameTeam.filter((player) => player.formationSlot.includes("CB"))
            : category === "fullback"
              ? sameTeam.filter((player) => ["LB", "RB", "LWB", "RWB"].includes(player.formationSlot))
              : category === "defensive-midfielder"
                ? sameTeam.filter((player) => player.formationSlot.includes("DM"))
                : category === "goalkeeper-foot"
                  ? sameTeam.filter((player) => player.role === "keeper")
                  : category === "pressured-clearance"
                    ? sameTeam.filter((player) => player.line === "defender")
                    : sameTeam.filter((player) => player.line === "forward" || player.line === "midfielder");
          const fallbackLine: PlayerLine = category === "attacking"
            ? "forward"
            : category === "goalkeeper-foot"
              ? "keeper"
              : category === "defensive-midfielder"
                ? "midfielder"
                : "defender";
          const passer = passerPool[testIndex % Math.max(passerPool.length, 1)]
            ?? sameTeam.find((player) => player.line === fallbackLine)
            ?? null;
          const receiverLine: PlayerLine = testIndex % 5 === 2 ? "defender" : testIndex % 3 === 0 ? "forward" : "midfielder";
          const receiver = active.players.find((player) => (
            player.team === passingTeam
            && player.line === receiverLine
            && player.id !== passer?.id
            && !player.sentOff
          ...
          if (passer && receiver) {
            const style: KickStyle = category === "goalkeeper-foot"
              ? "short"
              : category === "pressured-clearance"
                ? "long"
              : category === "attacking"
                ? (["short", "through", "low-through", "short"] as const)[testIndex % 4]
                : (["short", "short", "low-through", "short"] as const)[testIndex % 4];
            const attackSign = Math.sign(attackingGoalZ(passingTeam, active.half)) || 1;
            const backward = category !== "goalkeeper-foot"
              && category !== "pressured-clearance"
              && testIndex % 10 === 2;
            const lateral = (testIndex % 4 - 1.5) * 5.6;
            const passerDepth = category === "attacking" ? 16 : category === "goalkeeper-foot" ? -FIELD_L / 2 + 7 : -14;
            passer.pos.set(
              category === "fullback" ? Math.sign(lateral || 1) * Math.min(18, FIELD_W / 2 - 12) : -lateral * 0.25,
              0,
              attackSign * passerDepth,
            );
            ...
            if (pressureOpponent && (category === "pressured-clearance" || testIndex % 7 === 5)) {
            ...
            if (kicked) {
              pendingPassIntentCheck = {
                ...
                createdCount,
                style,
                closestApproachDistance: receiver.pos.distanceTo(active.ballPos),
                received: false,
        ...
        if (
          ...
          if (attacker && defender) {
            ...
            active.players.forEach((player, index) => {
              if (player.id === attacker.id || player.id === defender.id || player.role === "keeper") return;
              const testSide = player.team === "home" ? -1 : 1;
              player.pos.set(
                testSide * (FIELD_W / 2 - 5 - (index % 3) * 1.8),
                0,
                FIELD_L / 2 - 8 - (index % 8) * 7,
              );
              player.vel.set(0, 0, 0);
              player.actionCooldown = 0;
              player.kickTimer = 0;
            ...
            attacker.mesh.rotation.y = attacker.heading;
            attacker.mesh.position.copy(attacker.pos);
            const tackleBallPoint = attacker.pos.clone().add(attackDirection.clone().multiplyScalar(DRIBBLE_DISTANCE));
            defender.pos.copy(tackleBallPoint).add(attackDirection.clone().multiplyScalar(0.78));
            defender.pos.x += (testIndex % 2 === 0 ? -1 : 1) * 0.12;
            defender.vel.set(0, 0, 0);
            ...
            active.cooldown = 0;
            const tackled = attemptTackle(defender, active);
            active.renderer.domElement.dataset.lastTackleBoundaryCommitted = tackled ? "true" : "false";
            pendingTackleCheck = { attackerId: attacker.id, defenderId: defender.id, testIndex, at: now + 320 };
      ...
      if (shouldUpdateHud) {
        ...
        if (activeOfflineSettings.minimapEnabled) {
          setMinimapSnapshot(createMinimapSnapshot(active));
        }
      }
    ...
    const frame = (time: number) => {
      ...
      try {
        ...
        const firstPersonActive = active.cameraMode === "first-person"
          && active.state === "playing"
          && active.phase === "open"
          && Boolean(controlledFocus);
        const firstPersonForward = controlledFocus ? facingDirection(controlledFocus) : new THREE.Vector3(0, 0, -1);
        const firstPersonHead = new THREE.Vector3();
        if (firstPersonActive && controlledFocus) {
          const head = controlledFocus.parts.headContact ?? controlledFocus.parts.head;
          if (head) {
            head.updateWorldMatrix(true, false);
            head.getWorldPosition(firstPersonHead);
          } else {
            firstPersonHead.copy(controlledFocus.pos).add(new THREE.Vector3(0, 2.15, 0));
          }
        }
        const blendedFocus = active.ballPos.clone().lerp(playerFocus, 0.22);
        const requestedFocusZ = clamp(blendedFocus.z, -FIELD_L / 2 - 10, FIELD_L / 2 + 10);
        ...
        const baseTouchlineBlend = shouldFollowPlay ? cameraSideTouchlineBlend(touchlineFocusX) : 0;
        const previewCameraX = -FIELD_W / 2 - THREE.MathUtils.lerp(
          ADAPTIVE_TOUCHLINE_CAMERA.normalCameraDistance,
          ADAPTIVE_TOUCHLINE_CAMERA.nearTouchlineMaximumOutwardDistance,
          baseTouchlineBlend,
        );
        const previewCameraY = ADAPTIVE_TOUCHLINE_CAMERA.normalCameraHeight
          + ADAPTIVE_TOUCHLINE_CAMERA.maximumAdditionalHeight * baseTouchlineBlend;
        const previewLookAtX = touchlineFocusX
          * ADAPTIVE_TOUCHLINE_CAMERA.lookAtLateralFollowStrength
          * baseTouchlineBlend;
        const previewViewportY = shouldFollowPlay
          ? projectedViewportY(
              touchlineFocus.x,
              0,
              touchlineFocus.z,
              previewCameraX,
              previewCameraY,
              active.cameraFocusZ + BROADCAST_CAMERA_Z_OFFSET,
              previewLookAtX,
              BROADCAST_LOOK_AT_Y,
              active.cameraFocusZ,
              active.camera.fov,
        ...
        const safetyBlend = THREE.MathUtils.smoothstep(
          previewViewportY,
          maximumViewportY,
          maximumViewportY + 0.12,
        );
        const requestedTouchlineBlend = clamp(
          baseTouchlineBlend + safetyBlend * (1 - baseTouchlineBlend),
          0,
          1,
        );
        if (!shouldFollowPlay) {
          active.cameraFocusZ = requestedFocusZ;
          active.cameraTouchlineBlend = 0;
        } else {
          const focusDifference = requestedFocusZ - active.cameraFocusZ;
          active.cameraFocusZ += focusDifference * (1 - Math.exp(-CAMERA_FOLLOW_DAMPING * dt));
          const blendDifference = requestedTouchlineBlend - active.cameraTouchlineBlend;
          active.cameraTouchlineBlend += blendDifference * (1 - Math.exp(-CAMERA_TOUCHLINE_DAMPING * dt));
        }
        const focusZ = active.cameraFocusZ;
        const touchlineBlend = clamp(active.cameraTouchlineBlend, 0, 1);
        const adaptiveCameraX = -FIELD_W / 2 - THREE.MathUtils.lerp(
          ADAPTIVE_TOUCHLINE_CAMERA.normalCameraDistance,
          ADAPTIVE_TOUCHLINE_CAMERA.nearTouchlineMaximumOutwardDistance,
          touchlineBlend,
        );
        const adaptiveCameraY = ADAPTIVE_TOUCHLINE_CAMERA.normalCameraHeight
          + ADAPTIVE_TOUCHLINE_CAMERA.maximumAdditionalHeight * touchlineBlend;
        const adaptiveLookAtX = touchlineFocusX
          * ADAPTIVE_TOUCHLINE_CAMERA.lookAtLateralFollowStrength
          * touchlineBlend;
        const inspectPlayer = modelInspectView
          ? active.players.find((player) => player.role === "field")
            ?? null
          : null;
        const inspectForward = inspectPlayer ? facingDirection(inspectPlayer) : null;
        const inspectRight = inspectForward ? new THREE.Vector3(inspectForward.z, 0, -inspectForward.x) : null;
        const inspectOffset = inspectPlayer && inspectForward && inspectRight
          ? modelInspectView === "side"
            ? inspectRight.multiplyScalar(6.3)
            : inspectForward.multiplyScalar(modelInspectView === "rear" ? -6.3 : 6.3)
          : null;
        const desired = firstPersonActive
          ? firstPersonHead.clone().addScaledVector(firstPersonForward, 0.34).add(new THREE.Vector3(0, 0.1, 0))
          : inspectPlayer && inspectOffset
          ? inspectPlayer.pos.clone().add(inspectOffset).add(new THREE.Vector3(0, 2.8, 0))
          : active.phase === "goal"
        ...
        const desiredLookAt = firstPersonActive
          ? firstPersonHead.clone().addScaledVector(firstPersonForward, 18).add(new THREE.Vector3(0, 0.08, 0))
          : inspectPlayer
        ...
        active.cameraLookAt.lerp(
          desiredLookAt,
          firstPersonActive
            ? 1 - Math.exp(-16 * dt)
            : active.phase === "goal"
              ? 1 - Math.pow(0.00003, dt)
              : shouldFollowPlay
                ? 1 - Math.pow(0.0016, dt)
                : 1,
        );
        const targetFov = firstPersonActive ? 72 : resolveCameraFov(mount.clientWidth);
        if (Math.abs(active.camera.fov - targetFov) > 0.05) {
          active.camera.fov = THREE.MathUtils.lerp(active.camera.fov, targetFov, 1 - Math.exp(-9 * dt));
          active.camera.updateProjectionMatrix();
        }
        active.camera.up.set(0, 1, 0);
        active.camera.lookAt(active.cameraLookAt);
        active.renderer.domElement.dataset.cameraX = active.camera.position.x.toFixed(3);
        active.renderer.domElement.dataset.cameraY = active.camera.position.y.toFixed(3);
        active.renderer.domElement.dataset.cameraZ = active.camera.position.z.toFixed(3);
        active.renderer.domElement.dataset.cameraFocusZ = active.cameraFocusZ.toFixed(3);
        active.renderer.domElement.dataset.cameraTouchlineBlend = active.cameraTouchlineBlend.toFixed(3);
        const rendererStartedAt = performance.now();
        const hiddenFirstPersonPlayer = firstPersonActive && controlledFocus ? controlledFocus.mesh : null;
        if (hiddenFirstPersonPlayer) hiddenFirstPersonPlayer.visible = false;
        try {
          active.renderer.render(active.scene, active.camera);
        } finally {
          if (hiddenFirstPersonPlayer) hiddenFirstPersonPlayer.visible = true;
        }
    ...
    return () => {
      ...
      runtime.scene.userData.disposed = true;
      runtime.players.forEach((player) => {
        player.locomotionController?.dispose();
        player.locomotionController = null;
      });
  ...
  useEffect(() => {
    const down = (event: KeyboardEvent) => {
      ...
      if (!typing && !event.repeat && event.code === "KeyF" && !detectAppleMobile()) {
        void requestGameFullscreen();
        event.preventDefault();
        return;
      }
      if (typing || settingsOpenRef.current) return;
      ...
      if (active?.state === "playing" && !event.repeat && event.code === "KeyC") {
        toggleCameraMode();
        event.preventDefault();
        return;
      }
      ...
      if (active && manualRestartInput && ["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(event.code)) {
        if (!event.repeat && active.phase === "corner") navigateManualRestartSelection(active, event.code);
        event.preventDefault();
        return;
      }
      ...
      if (!event.repeat && active?.state === "playing" && active.phase === "open") {
        ...
        const p1 = active.players.find((player) => player.controlledBy === "p1");
        if (event.code === "KeyW") {
          const executed = Boolean(p1 && active.ballOwnerId === p1.id
            && active.possession === "home"
            && performPass(p1, active, "through"));
          const buffered = !executed && bufferUserPassBeforeReception(active, keysRef.current, "through");
          active.renderer.domElement.dataset.lastThroughPassKey = "KeyW";
          active.renderer.domElement.dataset.lastThroughPassStatus = executed ? "executed" : buffered ? "buffered" : "rejected";
          event.preventDefault();
          return;
        }
        ...
        if (event.code === "KeyS") {
          ...
          if (defending) {
            ...
            active.passInputDownAt = 0;
            active.renderer.domElement.dataset.lastManualSwitchKey = "KeyS";
            event.preventDefault();
            return;
          }
          ...
          } else if (bufferUserPassBeforeReception(active, keysRef.current)) {
            active.renderer.domElement.dataset.lastPassInputStatus = "buffered-for-reception";
          } else {
      ...
      if (["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "KeyA", "KeyC", "KeyD", "KeyE", "KeyF", "KeyS", "KeyU", "KeyW", "KeyZ", "Space"].includes(event.code)) event.preventDefault();
    };
    const up = (event: KeyboardEvent) => {
      ...
      if (
        event.code === "KeyS"
        && active?.state === "playing"
        && (active.passChargingPlayerId !== null || active.passInputDownAt > 0 || active.pendingReceiveAction?.requestedBy === "user")
      ) {
        const buffered = active.pendingReceiveAction?.requestedBy === "user"
          && performance.now() <= active.pendingReceiveAction.expiresAt;
        const executed = buffered ? false : releasePassCharge(active, keysRef.current);
        if (executed) {
        ...
        } else if (!buffered && active.passInputDownAt > 0 && !active.renderer.domElement.dataset.lastPassInputStatus?.startsWith("rejected:")) {
          active.passInputRejected += 1;
          active.renderer.domElement.dataset.passInputRejected = String(active.passInputRejected);
  ...
  }, [requestGameFullscreen, syncP1AiUi, toggleCameraMode]);

  return (
    <main
      ref={gameRootRef}
      className="relative min-h-[100dvh] overflow-hidden bg-[#07110c] text-white"
          ...
          <div className="mx-auto my-4 w-full max-w-5xl rounded-md border border-white/15 bg-[#08130d]/95 p-4 shadow-2xl sm:p-5">
            <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="flex gap-2">
                ...
                <button
                  className="rounded-md bg-emerald-300 px-4 py-2 text-sm font-black text-slate-950 hover:bg-emerald-200 disabled:cursor-not-allowed disabled:bg-white/15 disabled:text-white/35"
            ...
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="grid grid-cols-2 gap-2" role="group" aria-label="Camera mode">
                  {(["broadcast", "first-person"] as const).map((mode) => (
                    <button
                      key={mode}
                      type="button"
                      className={`rounded border px-3 py-2 font-black ${cameraModeUi === mode ? "border-emerald-200 bg-emerald-300 text-slate-950" : "border-white/15 bg-black/20 text-white/70"}`}
                      onClick={() => {
                        const active = sceneRef.current;
                        if (!active || active.cameraMode === mode) return;
                        toggleCameraMode();
                      }}
                    >
              ...
              <label className="rounded-md border border-white/10 bg-white/5 p-3 text-xs font-bold text-white/75">
                ...
                <input
                  ...
                  onChange={(event) => setDraftSettings((current) => ({ ...current, homeColor: event.target.value }))}
                />
              </label>
              <label className="flex items-center justify-between gap-4 rounded-md border border-white/10 bg-white/5 p-3 text-xs font-bold text-white/75">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setDraftSettings((current) => ({ ...current, minimapEnabled: enabled }));
                    setOfflineSettings((current) => {
                      const next = { ...current, minimapEnabled: enabled };
                      activeOfflineSettings = next;
                      saveOfflineSettings(next);
                      return next;
                    });
                    const active = sceneRef.current;
                    setMinimapSnapshot(active && enabled ? createMinimapSnapshot(active) : null);
                  }}
                />
              </label>
              <label className="rounded-md border border-white/10 bg-white/5 p-3 text-xs font-bold text-white/75">
                ...
                <input
                  ...
                  onChange={(event) => setDraftSettings((current) => ({ ...current, crowdVolume: Number(event.target.value) }))}
                />
              </label>
            </div>
...
function integrateFreeBallPhysics(active: MatchRuntime, frameDt: number) {
  active.ballPhysicsAccumulator = Math.min(
    active.ballPhysicsAccumulator + Math.min(frameDt, BALL_PHYSICS_STEP * BALL_MAX_SUBSTEPS),
  ...
  while (active.ballPhysicsAccumulator >= BALL_PHYSICS_STEP && substeps < BALL_MAX_SUBSTEPS) {
    ...
    stepStart.copy(active.ballPos);
    active.previousBallPhysicsPos.copy(active.ballPos);

    const physicsResult = stepBallPhysics({
      position: active.ballPos,
      velocity: active.ballVel,
      angularVelocity: active.ballAngularVelocity,
      curve: active.ballCurve,
      grounded: active.ballGrounded,
    }, dt, SHARED_BALL_PHYSICS);
    active.ballGrounded = physicsResult.grounded;
    active.ballFrictionState = physicsResult.frictionState;
    if (physicsResult.bounced) active.ballCollisionCount += 1;

    if (!active.trajectoryDiagnosticActive) {
...
function updateMatch(
  ...
  active.passCommitTimer = Math.max(0, active.passCommitTimer - dt);
  active.shotInputLockTimer = Math.max(0, active.shotInputLockTimer - dt);
  active.restartBoundaryGuardTimer = Math.max(0, active.restartBoundaryGuardTimer - dt);
  updateManualGoalKickTarget(active, keys, dt);
  ...
  if (active.passChargingPlayerId) {
    ...
    const committedContact = Boolean(
      chargingPlayer
      && active.passCommitTimer > 0
      && chargingPlayer.pos.distanceTo(active.ballPos.clone().setY(0))
        <= playerBallContactRadius(chargingPlayer, active.ballPos.y) + 0.28,
    );
    if (!chargingPlayer || active.phase !== "open" || active.ballOwnerId !== chargingPlayer.id && !committedContact || !keys.has("KeyS")) {
      active.renderer.domElement.dataset.passCommitCancellationReason = !chargingPlayer
        ? "missing-player"
        : active.phase !== "open"
          ? `phase:${active.phase}`
          : active.ballOwnerId !== chargingPlayer.id && !committedContact
            ? `ownership:${active.ballOwnerId ?? "none"}`
            : "key-released-before-handler";
      active.passCharge = 0;
      active.passChargingPlayerId = null;
      active.passCommitTimer = 0;
    } else {
  ...
  updatePendingKickContact(active, dt);
  ...
  active.players.forEach((player) => {
    ...
    const sprintRequested = input.sprint || player.supportRunTimer > 0;
    if (player.stamina <= 0.001) player.sprintExhausted = true;
    if (player.sprintExhausted && !sprintRequested && player.stamina >= STAMINA_RECOVERY_THRESHOLD) {
      player.sprintExhausted = false;
    }
    const sprint = sprintRequested && hasMoveIntent && !player.sprintExhausted && player.stamina > 0;
    const defensiveRecovery = !manualControlled
      && defendingUnderTeamPlan
      && (defensivePlanRole === "press" || defensivePlanRole === "cover" || player.recoveryTimer <= 0);
    const maximumSpeedScale = committedReception && player.role === "keeper"
      ? 1.22
      : defensiveRecovery
        ? DEFENSIVE_RECOVERY_SPEED
        : 1;
    const speedScale = clamp(input.speedScale ?? 1, 0.35, maximumSpeedScale);
    const keeperBurst = player.role === "keeper" && (player.keeperAction === "intercept" || player.keeperAction === "smother");
    const maxSpeed = player.role === "keeper"
      ? (keeperBurst ? 8.1 : 5.8) * (sprint ? 1.18 : 1)
      : sprint
        ? SPRINT_SPEED
        : PLAYER_NORMAL_SPEED;
    player.stamina = clamp(
      player.stamina + (sprint ? -STAMINA_DRAIN_RATE : STAMINA_RECOVERY_RATE) * dt,
      0,
      1,
    );
    if (player.stamina <= 0.001) player.sprintExhausted = true;
    ...
    if (!manualControlled && defendingUnderTeamPlan && active.defensivePlan) {
      ...
      if (markingIncomingReceiver && markedOpponent) {
        ...
        if (faceDirection.lengthSq() > 0.05) {
          const bodyDirection = defensiveFacingDirection(player, player.vel, faceDirection);
          const runningWithTravel = bodyDirection.dot(faceDirection.normalize()) < 0.995;
          setPlayerHeading(
            player,
            headingFromDirection(bodyDirection),
            dt,
            runningWithTravel ? 7.2 : player.line === "defender" ? 13.5 : 8.5,
          );
        }
      }
    }
  ...
  if (!outOfPlayPending && !active.trajectoryDiagnosticActive && resolveBallBoundary(active)) return;
  if (!outOfPlayPending && !active.trajectoryDiagnosticActive) {
  ...
  if (active.phase === "open" && !outOfPlayPending && !active.tutorial.active && !active.trajectoryDiagnosticActive) {
  ...
  if (active.phase === "open" && dribbler) {
    const preparingKick = active.pendingKickContact?.playerId === dribbler.id
      ? active.pendingKickContact
      : null;
    if (preparingKick) {
      ball.copy(preparingKick.contactPosition);
      ballVel.set(0, 0, 0);
      active.ballCurve.set(0, 0, 0);
      active.ballAngularVelocity.set(0, 0, 0);
      active.ballGrounded = preparingKick.grounded;
      active.ballFrictionState = "sleeping";
    } else {
  ...
  if (!active.ballOwnerId && active.phase === "open" && !outOfPlayPending && !active.trajectoryDiagnosticActive) {
  ...
  if (!active.ballOwnerId && active.phase === "open" && !outOfPlayPending && !active.trajectoryDiagnosticActive) {
    ...
    let controlResolved = sweptReceptionResolved || (intendedAerialReceiver
      ? tryHeader(intendedAerialReceiver, active, dt) || tryAerialFirstTouch(intendedAerialReceiver, active, dt)
      : false);
    if (controller) {
    ...
    if (!controlResolved && !active.ballOwnerId && contacts.length > 0) {
      ...
      if (!globalContactCooldown) {
        ...
        recordBallTouch(active, player, "other", "deflection", false);
  ...
  if (!active.ballOwnerId) {
    if (integrateFreeBallPhysics(active, dt)) {
      syncBallVisual(active);
      return;
    }
  } else {
    active.ballPhysicsAccumulator = 0;
    active.ballGrounded = active.ballPos.y <= BALL_ENTER_GROUNDED_THRESHOLD;
    active.previousBallPhysicsPos.copy(active.ballPos);
  }
...
function trySweptPassInterception(active: MatchRuntime, start: THREE.Vector3, end: THREE.Vector3) {
  if (active.trajectoryDiagnosticActive) return false;
  if (
    active.phase !== "open"
    || active.ballOwnerId
    || active.ballState !== "kicked"
  ...
  if (!controlled) {
    ...
    active.ballState = "loose";
    recordBallTouch(active, contact.player, "other", "deflection", false);
...
function setManualRestartSelection(active: MatchRuntime, option: ManualRestartOption | null) {
  ...
  if (active.phase === "goal-kick" && option) {
    active.manualGoalKickTarget = option.target.clone().setY(BALL_RADIUS);
  }
...
function ensureManualRestartSelection(active: MatchRuntime) {
  ...
  if (active.phase === "goal-kick" && active.manualGoalKickTarget) {
    active.eventText = "GOAL KICK · MOVE TARGET · HOLD A";
    return;
  }
...
function updateManualGoalKickTarget(active: MatchRuntime, keys: Set<string>, dt: number) {
  if (
  ...
  ensureManualRestartSelection(active);
  const target = active.manualGoalKickTarget;
  if (!target) return;
  ...
  if (horizontal !== 0 || vertical !== 0) {
    const axes = cameraRelativeAxis(active.camera);
    const movement = axes.right.multiplyScalar(horizontal).addScaledVector(axes.up, vertical);
    if (movement.lengthSq() > 0.001) {
      movement.normalize();
      target.addScaledVector(movement, GOAL_KICK_CURSOR_SPEED * dt);
      target.x = clamp(target.x, -FIELD_W / 2 + 1.2, FIELD_W / 2 - 1.2);
      target.z = clamp(target.z, -FIELD_L / 2 + 1.2, FIELD_L / 2 - 1.2);
  ...
  active.renderer.domElement.dataset.manualGoalKickTargetX = target.x.toFixed(3);
  active.renderer.domElement.dataset.manualGoalKickTargetZ = target.z.toFixed(3);
  active.renderer.domElement.dataset.manualGoalKickCursorSpeed = GOAL_KICK_CURSOR_SPEED.toFixed(2);
}
...
function chooseGoalKickPlan(
  active: MatchRuntime,
  keeper: PlayerBody | null,
  direction: THREE.Vector3,
): GoalKickPlan {
...
function executeSimpleGoalKick(
  active: MatchRuntime,
  actor: PlayerBody | null,
  manualCharge?: number,
  manualTarget?: THREE.Vector3,
) {
  const direction = upfieldKickDirection(active.restartTeam, active.half);
  ...
  if (manualTarget) {
    plan.target.copy(manualTarget).setY(BALL_RADIUS);
    plan.target.x = clamp(plan.target.x, -FIELD_W / 2 + 1.2, FIELD_W / 2 - 1.2);
    plan.target.z = clamp(plan.target.z, -FIELD_L / 2 + 1.2, FIELD_L / 2 - 1.2);
    const sameTeamReceiver = active.players
      .filter((player) => player.team === active.restartTeam && player.role !== "keeper" && !player.sentOff)
      .sort((left, right) => left.pos.distanceToSquared(plan.target) - right.pos.distanceToSquared(plan.target))[0] ?? null;
    plan.receiver = sameTeamReceiver && sameTeamReceiver.pos.distanceTo(plan.target) <= 5.5
      ? sameTeamReceiver
      : null;
    plan.targetDistance = plan.target.distanceTo(active.ballPos);
    plan.safetyScore = plan.receiver ? nearestOpponentDistance(plan.receiver, active.players) : 0;
    plan.laneBlockers = keeper ? opponentsBetween(keeper, plan.target, active.players, 4.1) : 0;
    plan.receiverPressure = opponentPressureAtPoint(active.restartTeam, plan.target, active.players, 7.5);
    plan.landingPressure = opponentPressureAtPoint(active.restartTeam, plan.target, active.players, 5.5);
  }
  ...
  active.ballVel.set(0, 0, 0);
  if (manualTarget) {
    const landingSolution = solveBallisticLandingVelocity(
      active.ballPos,
      plan.target,
      SHARED_BALL_PHYSICS,
      {
        preferredHorizontalSpeed: THREE.MathUtils.lerp(23, 34, clamp(manualCharge ?? 0.5, 0, 1)),
        minimumFlightTime: 0.9,
        maximumFlightTime: 5.5,
      },
    );
    active.goalKickPendingVelocity.copy(landingSolution.velocity);
    active.renderer.domElement.dataset.goalKickPredictedLandingError = landingSolution.error.toFixed(3);
    active.renderer.domElement.dataset.goalKickPredictedLandingX = landingSolution.predictedLandingPoint.x.toFixed(3);
    active.renderer.domElement.dataset.goalKickPredictedLandingZ = landingSolution.predictedLandingPoint.z.toFixed(3);
  } else {
...
function resumeRestart(active: MatchRuntime) {
  ...
  if (restartingPhase === "goal-kick") {
    ...
    if (manualGoalKick && !active.manualGoalKickTarget) {
      active.phaseTimer = 0.12;
      active.eventText = "GOAL KICK · MOVE TARGET · HOLD A";
      active.eventTimer = 0;
      active.ballVel.set(0, 0, 0);
  ...
  if (actor) {
    recordBallTouch(
      active,
      actor,
      restartingPhase === "throw-in" ? "other" : "foot",
      restartingPhase === "throw-in" ? "throwIn" : "pass",
      true,
      active.intendedReceiverId ?? undefined,
    );
  } else {
    active.lastTouchTeam = active.restartTeam;
...
function forceKickoffPass(actor: PlayerBody, active: MatchRuntime) {
  ...
  if (!kicked && passDirection.lengthSq() > 0.05) {
    ...
    recordBallTouch(active, actor, "foot", "pass", true, supportTarget.id);
...
function clearPassIntent(
  ...
  if (
    result !== "resolved"
    && clearedReceiverId
    && active.pendingReceiveAction?.receiverId === clearedReceiverId
  ) {
    active.pendingReceiveAction = null;
  }
...
function receiveActionTarget(
  receiver: PlayerBody,
  passer: PlayerBody | null,
  active: MatchRuntime,
  preferredDirection?: THREE.Vector3,
) {
  const attackSign = Math.sign(attackingGoalZ(receiver.team, active.half));
  return active.players
    .filter((candidate) => (
      candidate.team === receiver.team
      && candidate.id !== receiver.id
      && candidate.id !== passer?.id
      && !candidate.sentOff
      && candidate.role !== "keeper"
    ))
    .map((candidate) => {
      const target = kickTargetForStyle(receiver, active, candidate, "short");
      const direction = target.clone().sub(receiver.pos).setY(0);
      const distance = direction.length();
      const alignment = preferredDirection && preferredDirection.lengthSq() > 0.05
        ? preferredDirection.clone().setY(0).normalize().dot(direction.clone().normalize())
        : 0.35;
      const blockers = opponentsBetween(receiver, target, active.players, 3.2);
      const pressure = opponentPressureAtPoint(receiver.team, target, active.players, 5.8);
      const forward = (candidate.pos.z - receiver.pos.z) * attackSign;
      const score = alignment * 8
        + clamp(forward, -8, 16) * 0.42
        + nearestOpponentDistance(candidate, active.players) * 1.7
        - blockers * 32
        - pressure * 18
        - Math.abs(distance - 18) * 0.32;
      return { candidate, target, direction, distance, alignment, blockers, pressure, score };
    })
    .filter(({ distance, alignment, blockers, pressure }) => (
      distance >= 6
      && distance <= 34
      && alignment > -0.2
      && blockers === 0
...
function prepareAiReceiveAction(
  active: MatchRuntime,
  passer: PlayerBody,
  receiver: PlayerBody,
  style: KickStyle,
...
function executePendingReceiveAction(active: MatchRuntime, receiver: PlayerBody, immediateOnly = false) {
  const pending = active.pendingReceiveAction;
  if (!pending || pending.receiverId !== receiver.id || performance.now() > pending.expiresAt) {
    if (pending?.receiverId === receiver.id) active.pendingReceiveAction = null;
    return false;
  }
  if (pending.action === "control") {
    active.pendingReceiveAction = null;
    return false;
  }
  ...
  const nextReceiver = pending.nextReceiverId
    ...
    : null;
  if (!nextReceiver || nextReceiver.team !== receiver.team || nextReceiver.sentOff) {
    active.pendingReceiveAction = null;
    return false;
  }
  const target = pending.targetPosition?.clone() ?? kickTargetForStyle(receiver, active, nextReceiver, "short");
  if (
    opponentsBetween(receiver, target, active.players, 3.2) > 0
    || opponentPressureAtPoint(receiver.team, target, active.players, 5.4) > 0
  ) {
    active.pendingReceiveAction = null;
    return false;
  }
  receiver.actionCooldown = 0;
  receiver.recoveryTimer = 0;
  const style = pending.passStyle ?? "short";
  ...
  const kicked = kickTowardPoint(
    ...
    style,
    nextReceiver,
    tacticalChargeForKick(style, resolvedTarget.distanceTo(receiver.pos)),
  );
  if (kicked) {
    active.renderer.domElement.dataset.lastReceiveAction = pending.action;
    active.renderer.domElement.dataset.lastReceiveActionReceiver = receiver.id;
    active.renderer.domElement.dataset.lastReceiveActionNextReceiver = nextReceiver.id;
  }
  active.pendingReceiveAction = null;
  return kicked;
}

function updatePendingReceiveAction(active: MatchRuntime) {
  const pending = active.pendingReceiveAction;
  if (!pending) return;
...
function beginPassIntent(
  ...
  const passType: PassIntent["passType"] = style === "short"
    ? "short"
    : style === "low-through"
      ? "driven"
      : style === "through"
...
function realHeaderContact(player: PlayerBody, active: MatchRuntime, dt: number): HeaderContact | null {
  ...
  const head = player.parts.headContact ?? player.parts.head;
  if (!head) {
  ...
  const ballEnd = ballStart.clone().addScaledVector(active.ballVel, travelDt);
  let headStart = player.previousHeadWorldPosition.clone();
  if (player.headerSampleFrame < 0 || headStart.distanceTo(headPoint) > 2.4) headStart = headPoint.clone();
  if (player.headerSampleFrame !== active.frameCount) {
    player.previousHeadWorldPosition.copy(headPoint);
    player.headerSampleFrame = active.frameCount;
  }
  ...
  if (!contact) {
    const minimumSampleDistance = Math.min(
      ballStart.distanceTo(headStart),
      ballEnd.distanceTo(headPoint),
    );
    if (traceHeader) active.renderer.domElement.dataset.headerRejectReason = `distance:${minimumSampleDistance.toFixed(3)}`;
    return null;
  }
  const contactBallPoint = contact.firstPoint;
  const contactHeadPoint = contact.secondPoint;
...
function tryHeader(player: PlayerBody, active: MatchRuntime, dt = 1 / 60) {
  ...
  const action: BallTouchAction = ownGoalDistance < 30 ? "clearance" : Math.abs(goalZ - player.pos.z) < 35 ? "shot" : "pass";
  recordBallTouch(active, player, "head", action, true);
...
function tryAerialHeaderDuel(active: MatchRuntime, dt: number) {
  ...
  recordBallTouch(active, winner, "head", winner.line === "defender" ? "clearance" : "pass", true, active.intendedReceiverId ?? undefined);
  ...
  active.players
    ...
    .forEach((keeper) => {
      ...
      if (shotSpeed > 8) {
        ...
        recordBallTouch(active, keeper, "other", "deflection", false);
...
function defensiveFacingDirection(
  player: PlayerBody,
  travelDirection: THREE.Vector3,
  threatDirection: THREE.Vector3,
) {
  const travel = travelDirection.clone().setY(0);
  const threat = threatDirection.clone().setY(0);
  if (threat.lengthSq() < 0.05) return travel;
  threat.normalize();
  if (travel.lengthSq() < 0.05) return threat;
  travel.normalize();

  // Low-speed jockeying may stay square to the attacker. Once the defender is
  // running, the body must follow travel instead of playing a forward gait in
  // the opposite direction. The two thresholds give the choice hysteresis.
  const speed = Math.max(Math.hypot(player.vel.x, player.vel.z), player.previousHorizontalSpeed);
  const wasRunning = player.previousHorizontalSpeed > 5.05;
  const runningThreshold = wasRunning ? 4.45 : 5.25;
  const movingAwayFromThreat = travel.dot(threat) < 0.08;
  return speed >= runningThreshold || (speed >= 3.85 && movingAwayFromThreat) ? travel : threat;
}
...
function movePlayer(player: PlayerBody, moveDir: THREE.Vector3, maxSpeed: number, dt: number, active: MatchRuntime) {
  ...
  if (player.role === "keeper") {
    ...
    if (toCarrier.lengthSq() > 0.05) {
      desiredHeading = headingFromDirection(defensiveFacingDirection(player, dir, toCarrier));
    }
  } else if (hasIntent) {
...
function setJointDelta(joint: THREE.Object3D | null, x = 0, y = 0, z = 0) {
  if (!joint) return;
  const rest = joint.userData.restQuaternion as THREE.Quaternion | undefined;
  jointAnimationEuler.set(x, y, z);
  jointAnimationQuaternion.setFromEuler(jointAnimationEuler);
  if (rest) joint.quaternion.copy(rest).multiply(jointAnimationQuaternion);
  else joint.quaternion.copy(jointAnimationQuaternion);
}

function setJointRestPosition(joint: THREE.Object3D | null) {
  if (!joint) return;
  const rest = joint.userData.restPosition as THREE.Vector3 | undefined;
  if (rest) joint.position.copy(rest);
}

function locomotionStateForPlayer(
  player: PlayerBody,
  speed: number,
  acceleration: number,
  turnAngle: number,
  ...
  if (player.kickTimer > 0) return player.actionCooldown > 0.08 ? "shot-approach" : "pass-approach";
  if (player.recoveryTimer > 0 || player.tackleTimer > 0) return "recovery";
  if (speed < 0.18) return "idle";
  if (Math.abs(turnAngle) > 0.8) return "sharp-turn";
  if (forwardMotion < -0.2) return "backpedal";
  if (Math.abs(lateralMotion) > 0.58) return "lateral-shuffle";
  if (acceleration > 3.4) return "acceleration";
  if (acceleration < -3.8) return "deceleration";
  if (carryingBall) return speed > 7 ? "dribble-sprint" : "dribble-jog";
  if (speed < 2.2) return "walk";
  if (speed < 5.2) return "jog";
  if (speed < 8.6) return "run";
  return "sprint";
}

function strideLengthForLocomotion(
  state: LocomotionState,
  speed: number,
  carryingBall: boolean,
  turnAngle: number,
) {
  const openStride = state === "walk"
    ? LOCOMOTION_STRIDE_LENGTHS.walk
    : state === "jog"
      ? LOCOMOTION_STRIDE_LENGTHS.jog
      : state === "run"
        ? LOCOMOTION_STRIDE_LENGTHS.run
        : state === "sprint"
          ? LOCOMOTION_STRIDE_LENGTHS.sprint
          : speed < 2.2
            ? LOCOMOTION_STRIDE_LENGTHS.walk
            : speed < 5.2
              ? LOCOMOTION_STRIDE_LENGTHS.jog
              : speed < 8.6
                ? LOCOMOTION_STRIDE_LENGTHS.run
                : LOCOMOTION_STRIDE_LENGTHS.sprint;
  const directionFactor = state === "sharp-turn"
    ? 0.7
    : state === "lateral-shuffle" || state === "backpedal"
      ? 0.74
      : Math.abs(turnAngle) > 0.48
        ? 0.84
        : 1;
  const controlFactor = carryingBall ? 0.88 : 1;
...
function animatePlayer(player: PlayerBody, dt: number, camera?: THREE.Camera, active?: MatchRuntime) {
  ...
  const travelled = Math.hypot(
    player.pos.x - player.animationLastPos.x,
    player.pos.z - player.animationLastPos.z,
  );
  player.animationLastPos.copy(player.pos);
  player.distanceTravelled += travelled;
  const horizontalAcceleration = dt > 0 ? (horizontalSpeed - player.previousHorizontalSpeed) / dt : 0;
  player.previousHorizontalSpeed = horizontalSpeed;
  const velocityHeading = horizontalSpeed > 0.08 ? Math.atan2(player.vel.x, player.vel.z) : player.heading;
  const turnAngle = angleDelta(player.heading, velocityHeading);
  const nextLocomotionState = locomotionStateForPlayer(
    player,
    speed,
  ...
  if (nextLocomotionState !== player.locomotionState) {
    player.locomotionState = nextLocomotionState;
    player.locomotionBlend = 0;
  }
  player.locomotionBlend = Math.min(1, player.locomotionBlend + dt * 7.5);
  const strideLength = player.role === "keeper"
    ? strideLengthForLocomotion(nextLocomotionState, Math.min(speed, 7.6), false, turnAngle) * 0.86
    : strideLengthForLocomotion(nextLocomotionState, speed, carryingBall, turnAngle);
  if (moving && travelled > 0.0001) player.runPhase += travelled / strideLength * Math.PI * 2;
  else if (!moving) player.runPhase += dt * 0.72;
  ...
  const strideScale = (player.role === "keeper"
    ...
    * ballControlStride
    * (1 - shuffleBlend * 0.28);
  const stride = moving ? gaitSin * strideScale * (0.48 + locomotionBlend * 0.52) : 0;
  ...
  if (bodyRoot) {
    ...
    setJointDelta(
      bodyRoot,
      forwardMotion < -0.15 ? 0.025 : forwardLean,
      moving ? gaitSin * 0.018 * (1 - shuffleBlend) : 0,
      moving
  ...
  if (pelvis) {
    ...
    setJointDelta(
      pelvis,
      moving ? -0.014 * jogBlend : 0,
      moving ? gaitSin * (0.035 + jogBlend * 0.035 + sprintBlend * 0.025) : 0,
      moving ? gaitCos * (0.014 + sprintBlend * 0.018) + turnLean * 0.32 : 0,
    );
  }
  setJointDelta(
    spineLower,
    moving ? forwardLean * 0.24 : idleBreath * 0.3,
    moving ? gaitSin * (0.026 + sprintBlend * 0.018) : 0,
    moving ? gaitCos * 0.012 + turnLean * 0.22 : 0,
  );
  setJointDelta(
    spineUpper,
    moving ? forwardLean * 0.32 : idleBreath * 0.5,
    moving ? -gaitSin * (0.04 + sprintBlend * 0.025) : 0,
    moving ? -gaitCos * 0.012 - turnLean * 0.28 : 0,
  );
  if (chest) {
    setJointDelta(
      chest,
      moving ? -0.018 * jogBlend - sprintBlend * 0.024 : idleBreath * 0.8,
      moving ? -gaitSin * (0.035 + jogBlend * 0.035 + sprintBlend * 0.03) : 0,
      moving ? -gaitCos * (0.012 + sprintBlend * 0.012) - turnLean * 0.2 : 0,
    );
  }
  setJointDelta(
    neckBase,
    moving ? -forwardLean * 0.16 : 0,
    moving ? gaitSin * 0.012 : 0,
    moving ? -turnLean * 0.16 : 0,
  );
  setJointDelta(
    neckTop,
    moving ? -forwardLean * 0.1 : 0,
    moving ? -gaitSin * 0.008 : 0,
    moving ? -turnLean * 0.12 : 0,
  );
  setJointDelta(
    head,
    moving ? gaitDouble * 0.006 - forwardLean * 0.08 : 0,
    moving ? gaitSin * 0.008 : 0,
    moving ? -turnLean * 0.14 : 0,
  );
  setJointDelta(leftClavicle, 0, moving ? gaitSin * 0.018 : 0, moving ? -gaitCos * 0.012 : 0);
  setJointDelta(rightClavicle, 0, moving ? -gaitSin * 0.018 : 0, moving ? gaitCos * 0.012 : 0);
  setJointDelta(leftLeg, stride, 0, defensiveShuffle ? -0.09 - shuffleBlend * 0.04 : turnLean * -0.05);
  setJointDelta(rightLeg, -stride, 0, defensiveShuffle ? 0.09 + shuffleBlend * 0.04 : turnLean * -0.05);
  setJointDelta(leftKnee, leftKneeFlex, 0, 0);
  setJointDelta(rightKnee, rightKneeFlex, 0, 0);
  if (leftFoot) {
    const leftSwinging = moving && gaitSin > 0.04;
    const footPitch = leftSwinging
      ? clamp(-0.2 + leftKneeFlex * 0.34, -0.18, 0.3)
      : clamp(-stride * 0.24 - sprintBlend * 0.06, -0.22, 0.16);
    setJointDelta(leftFoot, footPitch, 0, 0);
  }
  if (rightFoot) {
    const rightSwinging = moving && gaitSin < -0.04;
    const footPitch = rightSwinging
      ? clamp(-0.2 + rightKneeFlex * 0.34, -0.18, 0.3)
      : clamp(stride * 0.24 - sprintBlend * 0.06, -0.22, 0.16);
    setJointDelta(rightFoot, footPitch, 0, 0);
  }
  if (leftArm) {
    setJointDelta(leftArm, leftArmSwing, 0, -(carryingBall ? 0.06 : 0) - shuffleBlend * 0.17 + (moving ? gaitSin * 0.012 : 0));
  }
  if (rightArm) {
    setJointDelta(rightArm, rightArmSwing, 0, (carryingBall ? 0.06 : 0) + shuffleBlend * 0.17 - (moving ? gaitSin * 0.012 : 0));
  }
  setJointDelta(leftElbow, clamp(0.12 + jogBlend * 0.2 + sprintBlend * 0.2 + Math.max(0, -leftArmSwing) * 0.18, 0.08, 1.18), 0, 0);
  setJointDelta(rightElbow, clamp(0.12 + jogBlend * 0.2 + sprintBlend * 0.2 + Math.max(0, -rightArmSwing) * 0.18, 0.08, 1.18), 0, 0);
  ...
  if (player.role === "keeper" && player.catchTimer <= 0 && player.diveTimer <= 0 && player.keeperAction === "none") {
    ...
    setJointDelta(leftArm, -0.32, 0, -0.3);
    setJointDelta(rightArm, -0.32, 0, 0.3);
    setJointDelta(leftElbow, 0.82, 0, 0);
    setJointDelta(rightElbow, 0.82, 0, 0);
    if (leftKnee) leftKnee.rotation.x += 0.14;
    if (rightKnee) rightKnee.rotation.x += 0.14;
  }
  ...
  if (player.passRequestTimer > 0 && player.kickTimer <= 0 && player.tackleTimer <= 0 && player.catchTimer <= 0) {
    ...
    setJointDelta(rightArm, -1.94 - requestPose * 0.34, 0, 0.46);
    setJointDelta(rightElbow, 0.18, 0, 0);
  }

  if (player.celebrateTimer > 0) {
    setJointDelta(leftArm, -2.45 + Math.sin(player.celebrateTimer * 9) * 0.18, 0, 0);
    setJointDelta(rightArm, -2.45 - Math.sin(player.celebrateTimer * 9) * 0.18, 0, 0);
    setJointDelta(leftElbow, 0.14, 0, 0);
    setJointDelta(rightElbow, 0.14, 0, 0);
  }

  if (player.kickTimer > 0) {
    const progress = clamp(1 - player.kickTimer / 0.46, 0, 1);
    const backswing = progress < 0.34 ? -Math.sin(progress / 0.34 * Math.PI / 2) * 0.92 : 0;
    const followThrough = progress >= 0.34 ? Math.sin((progress - 0.34) / 0.66 * Math.PI) * 1.42 : 0;
    const kickArc = backswing + followThrough;
    const kickingLeg = player.preferredFoot === "left" ? leftLeg : rightLeg;
    const kickingKnee = player.preferredFoot === "left" ? leftKnee : rightKnee;
    const kickingFoot = player.preferredFoot === "left" ? leftFoot : rightFoot;
    const plantLeg = player.preferredFoot === "left" ? rightLeg : leftLeg;
    const plantKnee = player.preferredFoot === "left" ? rightKnee : leftKnee;
    const plantFoot = player.preferredFoot === "left" ? rightFoot : leftFoot;
    if (kickingLeg) kickingLeg.rotation.x = kickArc;
    if (kickingKnee) kickingKnee.rotation.x = backswing < 0 ? 0.72 : Math.max(0.08, 0.36 - followThrough * 0.2);
    if (plantLeg) plantLeg.rotation.x = -0.16;
    if (plantKnee) plantKnee.rotation.x = 0.28;
    if (plantFoot) plantFoot.rotation.x = 0.12;
    if (kickingFoot) kickingFoot.rotation.x = -0.18 + followThrough * 0.12;
    if (pelvis) pelvis.rotation.y += 0.24 * Math.sin(progress * Math.PI);
    if (chest) {
      chest.rotation.y -= 0.2 * Math.sin(progress * Math.PI);
      chest.rotation.z += 0.08;
      chest.rotation.x -= 0.08 * Math.sin(progress * Math.PI);
    }
    if (bodyRoot) {
  ...
  if (player.headerTimer > 0) {
    ...
    if (bodyRoot) {
      bodyRoot.position.y += 0.38 * headerPose;
      bodyRoot.position.z -= 0.08 * headerPose;
      bodyRoot.rotation.x = -0.48 * headerPose;
      bodyRoot.rotation.z += (player.number % 2 === 0 ? 0.08 : -0.08) * headerPose;
    }
    setJointDelta(leftArm, -0.7 * headerPose, 0, -0.42);
    setJointDelta(rightArm, -0.7 * headerPose, 0, 0.42);
    if (chest) chest.rotation.x = -0.24 * headerPose;
    if (neckBase) neckBase.rotation.x = 0.22 * headerPose;
  ...
  if (player.firstTouchTimer > 0 && player.firstTouchType) {
    ...
    if (player.firstTouchType === "foot") {
      ...
      setJointDelta(leftArm, 0, 0, -0.34 * controlPose);
      setJointDelta(rightArm, 0, 0, 0.34 * controlPose);
    }
  }
  if (player.skillTimer > 0 && player.skillMove) {
    ...
    if (player.skillMove === "rainbow-flick") {
      ...
      setJointDelta(leftArm, 0, 0, -0.42);
      setJointDelta(rightArm, 0, 0, 0.42);
      ...
      setJointDelta(rightArm, 0, 0, 0.28);
      setJointDelta(leftArm, 0, 0, -0.28);
      ...
      setJointDelta(leftArm, 0, 0, -0.16 - side * 0.08);
      setJointDelta(rightArm, 0, 0, 0.16 - side * 0.08);
    }
  }
  if (player.catchTimer > 0) {
    const catchPose = Math.sin((1 - player.catchTimer / 0.62) * Math.PI);
    setJointDelta(leftArm, -1.12 - catchPose * 0.3, 0, -0.42);
    setJointDelta(rightArm, -1.12 - catchPose * 0.3, 0, 0.42);
    setJointDelta(leftElbow, 0.18, 0, 0);
    setJointDelta(rightElbow, 0.18, 0, 0);
    if (bodyRoot) bodyRoot.rotation.x = -0.08;
    if (chest) chest.rotation.x = -0.13 * catchPose;
  }
  if (player.diveTimer > 0) {
    ...
    setJointDelta(leftArm, -1.62, 0, -0.54 - visualDiveSide * 0.32);
    setJointDelta(rightArm, -1.62, 0, 0.54 - visualDiveSide * 0.32);
  ...
  if (player.role === "keeper" && player.keeperAction === "smother") {
    ...
    setJointDelta(leftArm, -1.72, 0, -0.3);
    setJointDelta(rightArm, -1.72, 0, 0.3);
  ...
  if (player.blockTimer > 0) {
    ...
    setJointDelta(leftArm, -1.18 * blockPose, 0, 0);
    setJointDelta(rightArm, -1.24 * blockPose, 0, 0);
  }
  if (player.tackleTimer > 0) {
    ...
    if (rightFoot) rightFoot.rotation.x = -0.22 * lunge;
    if (pelvis) pelvis.rotation.y += (player.number % 2 === 0 ? 0.16 : -0.16) * lunge;
    setJointDelta(leftArm, -0.82 * lunge, 0, 0);
    setJointDelta(rightArm, 0.46 * lunge, 0, 0);
  }
  // The forearms are local children of their upper arms. Keeping one positive
  // hinge convention on both sides prevents mirrored elbows from hyperextending.
  if (leftElbow) leftElbow.rotation.x = clamp(leftElbow.rotation.x, 0.08, 1.55);
  if (rightElbow) rightElbow.rotation.x = clamp(rightElbow.rotation.x, 0.08, 1.55);
}
...
function startMatchAudio(active: MatchRuntime) {
  ...
  if (active.audio.state === "running") start();
  else void active.audio.resume().then(start).catch(() => {
    active.renderer.domElement.dataset.crowdAmbience = "blocked";
  });
}
...
function playCommentary(active: MatchRuntime, cue: CommentaryCue, force = false) {
  void force;
  active.lastCommentaryCue = cue;
  active.renderer.domElement.dataset.commentary = "disabled-no-licensed-clips";
}
...
function playGoalSound(active: MatchRuntime) {
  ...
  playCommentary(active, "goal", true);
}
...
function playerInput(keys: Set<string>, _player: "p1", camera?: THREE.PerspectiveCamera): PlayerInputState {
  ...
  return { dir: dir.lengthSq() > 0 ? dir.normalize() : dir, sprint: keys.has("KeyE"), speedScale: 1 };
}
...
function cachedAiInput(player: PlayerBody, active: MatchRuntime, dt: number) {
  ...
  const assignedPrimaryPressure = Boolean(
    owner
    && owner.team !== player.team
    && active.defensivePlan?.defendingTeam === player.team
    && active.defensivePlan.primaryPresserId === player.id
    && distanceToBall <= DEFENSIVE_PRESSING_DISTANCE,
  );
  ...
  if (urgent || player.aiInputTimer <= 0) {
    ...
    const offBallInterval = distanceToBall > 42
      ...
      : distanceToBall > 26
        ? 0.3
        : 0.2;
    player.aiInputTimer = urgent ? DEFENSIVE_REACTION_DELAY : offBallInterval + (player.number % 5) * 0.018;
  }
...
function aiInput(player: PlayerBody, active: MatchRuntime) {
  ...
  } else if (isPressing && (opponentHasBall || distanceToBall < DEFENSIVE_PRESSING_DISTANCE)) {
  ...
  if (owner?.id === player.id) {
    ...
    const defensiveBuildupTarget = player.role === "keeper" || player.line === "defender"
      ? chooseDefensiveBuildupTarget(player, active)
      : null;
    const shortTarget = defensiveBuildupTarget ?? (player.role === "keeper" ? null : choosePassTarget(player, active, "short"));
    ...
    const passTarget = player.role === "keeper"
      ? defensiveBuildupTarget ?? choosePassTarget(player, active, passStyle)
      : forwardIsSafe
        ? forwardTarget
        : preferCurvedTarget
          ? curvedTarget
    ...
    if (player.decisionCooldown <= 0 && player.receiveControlTimer <= 0) {
      ...
      if (player.line === "defender" && pressured && ownGoalDistance < 24) {
        const safeOutlet = chooseDefensiveBuildupTarget(player, active);
        const hasSafeOutlet = Boolean(safeOutlet && passIsUseful(player, safeOutlet, active, "short") && nearestOpponentDistance(safeOutlet, active.players) > 5.6);
        acted = hasSafeOutlet && pressureCount < 3
          ? performValidatedAiPass(player, active, safeOutlet!, "short")
...
function tryRainbowFlick(player: PlayerBody, active: MatchRuntime, goalDistance: number) {
  ...
  active.ballIgnorePlayerId = player.id;
  active.ballIgnoreTimer = 0.24;
  recordBallTouch(active, player, "foot", "control", true, player.id);
...
function clearBall(player: PlayerBody, active: MatchRuntime) {
  ...
  const safeOutlet = chooseDefensiveBuildupTarget(player, active) ?? choosePassTarget(player, active, "short");
  const ownGoalDistance = Math.abs(teamGoalZ(player.team, active.half) - player.pos.z);
  ...
  const controlledClearance = chooseControlledClearanceTarget(player, active);
  if (controlledClearance) {
    return kickTowardPoint(
      player,
      controlledClearance.target,
      active,
      "long",
      controlledClearance.candidate,
      tacticalChargeForKick("long", controlledClearance.distance),
      false,
      "clearance",
    );
  }
...
function defensiveJockeyTarget(player: PlayerBody, active: MatchRuntime, carrier: PlayerBody, pressureIndex = 0) {
  ...
  const gap = stationary
    ? rank === 0 ? 0.95 : DEFENSIVE_MARKING_DISTANCE + 1.2
    : distanceFromGoal < 22
      ? rank === 0 ? DEFENSIVE_MARKING_DISTANCE * 0.82 : DEFENSIVE_MARKING_DISTANCE + 2
      : rank === 0 ? DEFENSIVE_MARKING_DISTANCE * 1.12 : DEFENSIVE_MARKING_DISTANCE + 3.2;
  ...
  if (rank === 0) stabilizePrimaryDefensiveTarget(target, player, carrier, active);
  target.x = clamp(target.x, -FIELD_W / 2 + 3, FIELD_W / 2 - 3);
  target.z = clamp(target.z, -FIELD_L / 2 + 4, FIELD_L / 2 - 4);
  return target;
}

function stabilizePrimaryDefensiveTarget(
  target: THREE.Vector3,
  player: PlayerBody,
  carrier: PlayerBody,
  active: MatchRuntime,
  ...
  if (toGoal.lengthSq() < 0.05) return target;
  toGoal.normalize();

  const desiredGap = target.clone().sub(carrierAnchor).setY(0).dot(toGoal);
  const currentGap = player.pos.clone().sub(carrierAnchor).setY(0).dot(toGoal);
  const carrierGoalwardSpeed = Math.max(0, carrier.vel.clone().setY(0).dot(toGoal));
  const spacingDeadZone = 0.9;
  const hasUsefulGoalSidePosition = currentGap > 0.72;
  const insideStableBand = Math.abs(currentGap - desiredGap) <= spacingDeadZone;
  const slowCarrierPushingBuffer = currentGap < desiredGap - spacingDeadZone && carrierGoalwardSpeed < 1.45;

  if (hasUsefulGoalSidePosition && (insideStableBand || slowCarrierPushingBuffer)) {
    // Preserve the defender's current pitch depth while still allowing lateral
    // jockeying. This prevents a walking attacker from translating the whole
    // defensive line goalward one frame at a time.
    target.addScaledVector(toGoal, currentGap - desiredGap);
  }

  const phase = defensiveDangerPhase(active, player.team, carrier);
  const tacticalAnchor = baseDefensiveShapeTarget(player, active, carrier, phase);
  const maximumGoalwardDrift = phase === "EMERGENCY_GOAL_DEFENSE" ? 6.2 : phase === "DEEP_BLOCK" ? 4.8 : 3.4;
  const goalwardDrift = target.clone().sub(tacticalAnchor).setY(0).dot(toGoal);
  if (goalwardDrift > maximumGoalwardDrift) {
    target.addScaledVector(toGoal, maximumGoalwardDrift - goalwardDrift);
  }
  return target;
}

function enforcePrimaryGoalSideContainment(
  target: THREE.Vector3,
  player: PlayerBody,
  carrier: PlayerBody,
  active: MatchRuntime,
  ...
  target.copy(carrierAnchor)
    .add(toGoal.multiplyScalar(goalSideGap))
    .add(sideAxis.multiplyScalar(lateralShade))
    .setY(0);
  return stabilizePrimaryDefensiveTarget(target, player, carrier, active);
}
...
function shouldStepInToTackle(player: PlayerBody, carrier: PlayerBody, active: MatchRuntime) {
  ...
  if (
    ballDistance <= 0.05
    || ballDistance > DEFENSIVE_INTERCEPTION_RANGE * (carrier.vel.length() < 0.9 ? 0.63 : 0.58)
  ) return false;
  ...
  const aggressionWindow = DEFENSIVE_TACKLE_AGGRESSION * 0.12;
  const canArriveFirst = defenderReachTime <= carrierRecoveryTime + aggressionWindow + (coverAvailable ? 0.18 : 0.08);
...
function separateDefensiveTargetLaterally(
  target: THREE.Vector3,
  player: PlayerBody,
  carrier: PlayerBody,
  ownZ: number,
  ...
  const relative = target.clone().sub(carrier.pos).setY(0);
  if (relative.length() >= minimumGap) return target;
  const goalSide = new THREE.Vector3(0, 0, ownZ).sub(carrier.pos).setY(0);
  if (goalSide.lengthSq() < 0.05) goalSide.set(0, 0, Math.sign(ownZ || 1));
  goalSide.normalize();
  const sideAxis = new THREE.Vector3(-goalSide.z, 0, goalSide.x);
  const depthGap = relative.dot(goalSide);
  const currentLateralGap = relative.dot(sideAxis);
  const requiredLateralGap = Math.sqrt(Math.max(0, minimumGap ** 2 - depthGap ** 2));
  const preferredSide = Math.sign(currentLateralGap || player.home.dot(sideAxis) || player.pos.dot(sideAxis) || 1);
  target.addScaledVector(sideAxis, preferredSide * requiredLateralGap - currentLateralGap);
  return target;
}
...
function updateDefensiveTeamPlan(active: MatchRuntime, dt: number) {
  ...
  if (!previousPressurePlan || !keepPreviousPrimary || (emergencyCover && !keepPreviousCover)) {
    active.defensivePlanTimer = DEFENSIVE_REACTION_DELAY;
  }
  ...
  remainingPlayers.forEach((player, index) => {
    ...
    separateDefensiveTargetLaterally(target, player, carrier, ownZ, minimumCarrierGap);
...
function enforceAntiSwarmInvariant(active: MatchRuntime, carrier: PlayerBody, plan: DefensiveTeamPlan) {
  ...
  extras.forEach((player) => {
    ...
    const escape = plan.targets.get(player.id)?.clone()
      ?? baseDefensiveShapeTarget(player, active, carrier, plan.dangerPhase);
    separateDefensiveTargetLaterally(escape, player, carrier, ownGoal.z, retreat);
...
function enforceDefensiveRuntimeGuard(active: MatchRuntime, dt: number) {
  ...
  extras.forEach((player) => {
    ...
    if (!escape || escape.distanceTo(carrier.pos) < minimumPlanGap) {
      escape ??= baseDefensiveShapeTarget(player, active, carrier, plan.dangerPhase);
      separateDefensiveTargetLaterally(escape, player, carrier, ownGoal.z, minimumPlanGap);
      escape.x = clamp(escape.x, -FIELD_W / 2 + 4, FIELD_W / 2 - 4);
      escape.z = clamp(escape.z, -FIELD_L / 2 + 5, FIELD_L / 2 - 5);
    ...
    if (escapeDirection.lengthSq() > 0.05) {
      ...
      if (faceCarrier.lengthSq() > 0.05) {
        const bodyDirection = defensiveFacingDirection(player, player.vel, faceCarrier);
        const runningWithTravel = bodyDirection.dot(faceCarrier.normalize()) < 0.995;
        setPlayerHeading(player, headingFromDirection(bodyDirection), dt, runningWithTravel ? 7.2 : 13.5);
      }
    }
...
function enforcePrimaryRuntimeContainment(active: MatchRuntime, dt: number) {
  ...
  enforcePrimaryGoalSideContainment(corridorTarget, primary, carrier, active, teamGoalZ(primary.team, active.half));
  const correction = corridorTarget.clone().sub(primary.pos).setY(0);
  const carrierGoalward = new THREE.Vector3(0, 0, teamGoalZ(primary.team, active.half))
    .sub(carrier.pos)
    .setY(0)
    .normalize();
  const carrierGoalwardSpeed = Math.max(0, carrier.vel.clone().setY(0).dot(carrierGoalward));
  const settledJockey = carrierGoalwardSpeed < 1.45 && correction.length() < 1.35;
  if (settledJockey) {
    primary.forcedMoveTimer = 0;
    primary.forcedMoveSprint = false;
  } else {
    primary.forcedMoveTarget.copy(corridorTarget);
    primary.forcedMoveTimer = Math.max(primary.forcedMoveTimer, 0.12);
    primary.forcedMoveSprint = correction.length() > 4;
  }

  if (!settledJockey && correction.lengthSq() > 0.04) {
  ...
  if (faceCarrier.lengthSq() > 0.04) {
    const bodyDirection = defensiveFacingDirection(primary, primary.vel, faceCarrier);
    const runningWithTravel = bodyDirection.dot(faceCarrier.normalize()) < 0.995;
    setPlayerHeading(primary, headingFromDirection(bodyDirection), dt, runningWithTravel ? 7.2 : 14.5);
  }
}
...
function defensiveTeamInput(player: PlayerBody, active: MatchRuntime, carrier: PlayerBody) {
  ...
  if (role === "press" && player.challengeCommitTimer <= 0) {
    enforcePrimaryGoalSideContainment(target, player, carrier, active, teamGoalZ(player.team, active.half));
  }
  const direction = target.sub(player.pos).setY(0);
  const carrierGoalward = new THREE.Vector3(0, 0, teamGoalZ(player.team, active.half))
    .sub(carrier.pos)
    .setY(0)
    .normalize();
  const carrierGoalwardSpeed = Math.max(0, carrier.vel.clone().setY(0).dot(carrierGoalward));
  if (
    role === "press"
    && player.challengeCommitTimer <= 0
    && carrierGoalwardSpeed < 1.45
    && direction.length() < 1.35
  ) {
    direction.set(0, 0, 0);
  }
...
function keepNonPressurePlayersOutOfDogpile(
  ...
  if (carrierToTarget.length() < minTargetGap) {
    separateDefensiveTargetLaterally(target, player, carrier, ownZ, minTargetGap);
  }
  const currentGap = player.pos.distanceTo(carrier.pos);
  if (currentGap < minTargetGap * 0.72) {
    separateDefensiveTargetLaterally(target, player, carrier, ownZ, minTargetGap * 0.72 + 1.8);
  }
  target.x = clamp(target.x, -FIELD_W / 2 + 3, FIELD_W / 2 - 3);
  target.z = clamp(target.z, -FIELD_L / 2 + 4, FIELD_L / 2 - 4);
}
...
function releasePossession(active: MatchRuntime, ballState: BallState) {
  ...
  if (ballState !== "kicked") active.pendingKickContact = null;
...
function updatePendingKickContact(active: MatchRuntime, dt: number) {
  const pending = active.pendingKickContact;
  if (!pending) return;
  pending.remaining = Math.max(0, pending.remaining - dt);
  if (pending.remaining > 0) return;

  const player = active.players.find((candidate) => candidate.id === pending.playerId) ?? null;
  const receiver = pending.receiverId
    ? active.players.find((candidate) => candidate.id === pending.receiverId) ?? null
    : null;
  const contactRange = player ? playerBallContactRadius(player, active.ballPos.y) + 0.34 : 0;
  const stillAtBall = Boolean(player && player.pos.distanceTo(active.ballPos.clone().setY(0)) <= contactRange);
  const stillOwnsBall = Boolean(player && active.ballOwnerId === player.id);
  active.pendingKickContact = null;
  if (!player || (!stillOwnsBall && !stillAtBall) || active.phase !== "open") {
    active.renderer.domElement.dataset.kickContactState = "cancelled-before-contact";
    return;
  }

  releasePossession(active, "kicked");
  active.ballPos.copy(pending.contactPosition);
  active.previousBallPhysicsPos.copy(active.ballPos);
  active.ballVel.copy(pending.velocity);
  active.ballCurve.copy(pending.curve);
  active.ballAngularVelocity.copy(pending.angularVelocity);
  active.ballGrounded = pending.grounded;
  if (Number(active.renderer.domElement.dataset.trajectoryTestsRequested ?? "0") > 0) {
    const shotStyle = pending.style === "shot" || pending.style === "driven" || pending.style === "finesse" || pending.style === "chip";
    const releaseTrajectory = simulateBallTrajectory({
      position: active.ballPos,
      velocity: active.ballVel,
      angularVelocity: active.ballAngularVelocity,
      curve: active.ballCurve,
      grounded: active.ballGrounded,
    }, BALL_PHYSICS_STEP, SHARED_BALL_PHYSICS, {
      maxSteps: 720,
      goalPlaneZ: shotStyle ? attackingGoalZ(player.team, active.half) : undefined,
      stopAtFirstLanding: pending.style === "long",
      stopSpeed: BALL_STOP_SPEED,
    });
    active.renderer.domElement.dataset.kickReleaseTrajectoryEndpoint = JSON.stringify(
      releaseTrajectory.endpoint.toArray().map((value) => Number(value.toFixed(4))),
    );
    active.renderer.domElement.dataset.kickReleasePosition = JSON.stringify(
      active.ballPos.toArray().map((value) => Number(value.toFixed(4))),
    );
    active.renderer.domElement.dataset.kickReleaseVelocity = JSON.stringify(
      active.ballVel.toArray().map((value) => Number(value.toFixed(4))),
    );
    active.renderer.domElement.dataset.kickReleaseCurve = JSON.stringify(
      active.ballCurve.toArray().map((value) => Number(value.toFixed(4))),
    );
  }
  active.manualPassReceiverId = !active.p1Autopilot && player.controlledBy === "p1" && receiver?.team === "home"
    ? receiver.id
    : null;
  if (isPassKickStyle(pending.style) && receiver) {
    beginPassIntent(active, player, receiver, pending.style, pending.direction, pending.target, pending.power);
  } else {
    clearPassIntent(active, "reset");
  }
  active.ballIgnorePlayerId = player.id;
  active.ballIgnoreTimer = pending.ignoreTimer;
  recordBallTouch(active, player, "foot", pending.touchAction, true, receiver?.id);
  playKickSound(active, pending.soundVolume);
  active.renderer.domElement.dataset.kickContactState = "released-at-contact";
}

function updatePendingKickTrajectory(
  active: MatchRuntime,
  velocity?: THREE.Vector3,
  curve?: THREE.Vector3,
) {
  const pending = active.pendingKickContact;
  if (!pending) return;
  if (velocity) pending.velocity.copy(velocity);
  if (curve) pending.curve.copy(curve);
  capBallVelocity(pending.velocity);
  pending.grounded = pending.velocity.y <= 0.65;
  pending.angularVelocity.set(
    pending.velocity.z / BALL_RADIUS,
    clamp(pending.curve.length() * (pending.direction.x >= 0 ? 1 : -1), -42, 42),
    -pending.velocity.x / BALL_RADIUS,
  ).multiplyScalar(pending.style === "driven" ? 0.9 : pending.style === "finesse" ? 1.28 : 0.72);
}
...
function updateKickTrajectoryPreview(
  ...
  intendedReceiver: PlayerBody | null = null,
) {
  ...
  if (style === "long" && intendedReceiver && player.role !== "keeper") {
    force = loftedPassForce(distance, charge, force);
  }
  ...
  velocity.y = assistedShot?.lift ?? force.lift;
  // Match the live launch path before integrating the preview.
  capBallVelocity(velocity);
  ...
  const start = active.ballPos.clone();
  ...
  const angularVelocity = new THREE.Vector3(
    ...
    clamp(curve.length() * (launchDirection.x >= 0 ? 1 : -1), -42, 42),
    -velocity.x / BALL_RADIUS,
  ).multiplyScalar(style === "driven" ? 0.9 : style === "finesse" ? 1.28 : 0.72);
  const sampleEvery = 8;
  const maximumSteps = pointCapacity * sampleEvery;
  const trajectory = simulateBallTrajectory({
    position: start,
    velocity,
    angularVelocity,
    curve,
    grounded: velocity.y <= 0.65 && start.y <= BALL_ENTER_GROUNDED_THRESHOLD,
  }, BALL_PHYSICS_STEP, SHARED_BALL_PHYSICS, {
    maxSteps: maximumSteps,
    sampleEvery,
    goalPlaneZ: undefined,
    targetDistance: style === "short" ? targetDistance : undefined,
    targetDirection: style === "short" ? targetDirection : undefined,
    stopAtFirstLanding: style === "long" || shotStyle,
    stopSpeed: BALL_STOP_SPEED,
  });
  const point = trajectory.endpoint;
  ...
  active.kickPreviewEndpoint.visible = false;
  active.kickLandingZone.visible = !intendedReceiver;
  if (!intendedReceiver) {
    const landing = landingPoint ?? point;
    active.kickLandingZone.position.set(landing.x, LANDING_MARKER_Y, landing.z);
    const markerScale = clamp(active.camera.position.distanceTo(active.kickLandingZone.position) / 68, 0.9, 1.45);
    active.kickLandingZone.scale.setScalar(markerScale);
  ...
  active.renderer.domElement.dataset.previewLaunchPosition = JSON.stringify(
    start.toArray().map((value) => Number(value.toFixed(4))),
  );
  active.renderer.domElement.dataset.previewLaunchVelocity = JSON.stringify(
    velocity.toArray().map((value) => Number(value.toFixed(4))),
  );
  active.renderer.domElement.dataset.previewLaunchCurve = JSON.stringify(
    curve.toArray().map((value) => Number(value.toFixed(4))),
  );
  active.renderer.domElement.dataset.previewIntendedX = resolvedTarget.x.toFixed(3);
  active.renderer.domElement.dataset.previewIntendedZ = resolvedTarget.z.toFixed(3);
  const comparisonPoint = style === "long" ? landingPoint ?? point : point;
  active.renderer.domElement.dataset.previewTargetError = Math.hypot(
    comparisonPoint.x - resolvedTarget.x,
    comparisonPoint.z - resolvedTarget.z,
  ).toFixed(3);
...
function updateAimIndicators(active: MatchRuntime, keys: Set<string>) {
  ...
  active.players.forEach((player) => {
    ...
    if (player.aimArrow) player.aimArrow.visible = false;
    const aiming = player.controlledBy === "p1"
      && active.state === "playing"
      && active.phase === "open"
      && !player.sentOff
      && active.ballOwnerId === player.id;
  ...
  const manualRestartActor = active.restartActorId
    ? active.players.find((player) => player.id === active.restartActorId) ?? null
    : null;
  const manualRestartTarget = active.phase === "goal-kick"
    ? active.manualGoalKickTarget
    : manualRestartOption?.target ?? null;
  if (manualRestartTarget && manualRestartActor) {
    const charge = active.loftChargingPlayerId === manualRestartActor.id ? clamp(active.loftCharge, 0.08, 1) : 0.08;
    if (active.phase === "goal-kick") {
      active.kickPreviewGuide.visible = false;
      active.kickPreviewLine.visible = false;
      active.kickPreviewEndpoint.visible = false;
      active.kickLandingZone.visible = true;
      active.kickLandingZone.position.set(manualRestartTarget.x, LANDING_MARKER_Y, manualRestartTarget.z);
      const markerScale = clamp(active.camera.position.distanceTo(active.kickLandingZone.position) / 68, 0.9, 1.45);
      active.kickLandingZone.scale.setScalar(markerScale);
      active.renderer.domElement.dataset.loftLandingX = manualRestartTarget.x.toFixed(3);
      active.renderer.domElement.dataset.loftLandingY = LANDING_MARKER_Y.toFixed(3);
      active.renderer.domElement.dataset.loftLandingZ = manualRestartTarget.z.toFixed(3);
      trajectoryVisible = true;
    } else if (manualRestartOption) {
      ...
      updateKickTrajectoryPreview(active, manualRestartActor, manualRestartTarget, "long", charge, new THREE.Vector3(), restartCurve);
      trajectoryVisible = true;
    }
  }
  ...
  active.players.forEach((player) => {
    ...
    player.receiverMarker.visible = selectedCornerTarget
    ...
    if (player.receiverMarker instanceof THREE.Mesh && player.receiverMarker.material instanceof THREE.MeshBasicMaterial) {
      player.receiverMarker.material.color.set(selectedCornerTarget ? "#facc15" : "#22d3ee");
      player.receiverMarker.material.opacity = selectedCornerTarget ? 1 : 0.9;
    }
  });
...
function handleAutomaticSteals(active: MatchRuntime) {
  if (active.passCommitTimer > 0 || active.trajectoryDiagnosticActive) return;
  const owner = ballOwner(active);
  ...
  const challengers = active.players
    ...
    .filter(({ player, distance }) => distance < (
      DEFENSIVE_INTERCEPTION_RANGE * (slowCarrier ? 0.59 : 0.375)
      + (isManualControlledPlayer(player, active) ? 0.34 : 0)
    ))
    .sort((a, b) => a.distance - b.distance)
    .slice(0, 2);
...
function handleFieldShotBlocks(active: MatchRuntime) {
  ...
  active.ballCurve.set(0, 0, 0);
  active.ballState = "loose";
  recordBallTouch(active, blockingPlayer, "other", "deflection", false);
...
function releaseShotCharge(active: MatchRuntime, keys: Set<string>) {
  ...
  if (kicked) {
    active.shotInputLockTimer = SHOT_INPUT_LOCK_SECONDS;
    player.actionCooldown = Math.max(player.actionCooldown, SHOT_INPUT_LOCK_SECONDS);
    player.vel.multiplyScalar(0.38);
    clampPlayer(player);
    player.mesh.position.copy(player.pos);
  }
  active.shotCharge = 0;
...
function bufferUserPassBeforeReception(
  active: MatchRuntime,
  keys: Set<string>,
  passStyle: "short" | "through" | "low-through" = "short",
) {
  if (active.ballState !== "kicked" || active.ballOwnerId) return false;
  const receiverId = active.passIntent?.receiverTeamId === "home"
    ? active.passIntent.receiverId
    : active.manualPassReceiverId
      ?? active.intendedReceiverId
      ?? active.receptionLockPlayerId;
  const receiver = receiverId
    ? active.players.find((candidate) => candidate.id === receiverId && candidate.team === "home" && !candidate.sentOff) ?? null
    : null;
  if (!receiver) return false;
  const aim = currentAimDirection(receiver, active, keys);
  const option = receiveActionTarget(receiver, null, active, aim);
  const now = performance.now();
  active.pendingReceiveAction = {
    receiverId: receiver.id,
    action: option ? "oneTouchPass" : "control",
    passStyle,
  ...
  active.renderer.domElement.dataset.userReceiveBuffer = option ? "one-touch-pass" : "control";
  active.renderer.domElement.dataset.userReceiveBufferReceiver = receiver.id;
  active.renderer.domElement.dataset.userReceiveBufferNext = option?.candidate.id ?? "";
  active.renderer.domElement.dataset.userReceiveBufferStyle = passStyle;
  return true;
}
...
function releasePassCharge(active: MatchRuntime, keys?: Set<string>) {
  ...
  const committedContact = Boolean(
    ...
    && active.phase === "open"
    && active.passCommitTimer > 0
    && player.pos.distanceTo(active.ballPos.clone().setY(0))
      <= playerBallContactRadius(player, active.ballPos.y) + 0.28,
  );
  if (!player || active.ballOwnerId !== player.id && !committedContact) {
    active.passCharge = 0;
    active.passChargingPlayerId = null;
    active.passCommitTimer = 0;
    return false;
  ...
  if (kicked) {
    ...
    const pendingVelocity = active.pendingKickContact?.velocity.clone() ?? new THREE.Vector3();
    pendingVelocity.y = 0;
    const pendingCurve = (curvedPass
      ? curvedPassSpin(player, resolvedAim.target, active, active.pendingKickContact?.velocity.length() ?? 0).add(assistCurve)
      : assistCurve).clone();
    updatePendingKickTrajectory(active, pendingVelocity, pendingCurve);
...
function releaseLoftCharge(active: MatchRuntime, keys?: Set<string>) {
  ...
  if (active.phase === "goal-kick" && active.restartTeam === "home" && !active.p1Autopilot) {
    const target = active.manualGoalKickTarget?.clone() ?? null;
    if (!target) return false;
    executeSimpleGoalKick(active, player, charge, target);
    active.renderer.domElement.dataset.lastManualGoalKickTarget = `${target.x.toFixed(2)},${target.z.toFixed(2)}`;
  ...
  if (kicked) {
    const pendingCurve = active.pendingKickContact?.curve.clone()
      .add(manualAimCurve(resolvedAim.finalDirection, resolvedAim.curveAssistRadians));
    if (pendingCurve) updatePendingKickTrajectory(active, undefined, pendingCurve);
...
function takePossession(player: PlayerBody, active: MatchRuntime, verifiedFirstTouch?: FirstTouchContact) {
  ...
  if (
    active.passCommitTimer > 0
    && active.passChargingPlayerId
    && player.id !== active.passChargingPlayerId
  ) return false;
  if (active.phase === "goal-kick") {
  ...
  const pendingAtContact = active.pendingReceiveAction?.receiverId === player.id
    && performance.now() <= active.pendingReceiveAction.expiresAt
    ? active.pendingReceiveAction
    : null;
  ...
  const controlBodyPart: BallTouchBodyPart = verifiedFirstTouch?.type === "thigh"
    ? "knee"
    : verifiedFirstTouch?.type === "chest"
      ? "chest"
      : "foot";
  if (restrictedBackPass) {
    active.lastTouchTeam = player.team;
    active.lastTouchPlayerId = player.id;
    active.renderer.domElement.dataset.keeperBackPassHandledWith = "foot";
    active.renderer.domElement.dataset.keeperBackPassRestrictedId = player.id;
  } else {
    recordBallTouch(active, player, controlBodyPart, "control", false);
  }
  ...
  if (pendingAtContact?.action === "oneTouchPass" && pendingAtContact.oneTouchEligible) {
    ...
    pendingAtContact.expiresAt = performance.now() + 520;
  } else if (pendingAtContact?.action === "control") {
    active.pendingReceiveAction = null;
  }
  return true;
}
...
function canControlBall(player: PlayerBody, active: MatchRuntime) {
  ...
  if (
    player.role === "keeper"
    && active.passIntent?.passerId === player.id
    && active.passIntent.receiverId !== player.id
  ) return false;
  const lockedReceiver = active.receptionLockPlayerId
    ? active.players.find((candidate) => candidate.id === active.receptionLockPlayerId) ?? null
    : null;
...
function keeperClaimTeamForLooseBall(active: MatchRuntime) {
  ...
  if (
    active.ballState === "kicked"
    && active.passIntent
    && active.passIntent.receiverTeamId === active.lastTouchTeam
  ) return null;
...
function kickTowardPoint(
  ...
  if (active.pendingKickContact) return rejectKick("kick-contact-pending");
  ...
  const kickVelocity = kickDirection.clone().multiplyScalar(kickPower)
    .add(player.vel.clone().multiplyScalar(style === "driven" ? 0.06 : 0.12));
  kickVelocity.y = force.lift;
  const kickGrounded = force.lift <= 0.65;
  const kickCurve = (assistedShot
    ? assistedShot.curve
    : curveForKick(style, player, kickDirection, resolvedTarget, { ...force, power: kickPower })).clone();
  const kickAngularVelocity = new THREE.Vector3(
    kickVelocity.z / BALL_RADIUS,
    clamp(kickCurve.length() * (kickDirection.x >= 0 ? 1 : -1), -42, 42),
    -kickVelocity.x / BALL_RADIUS,
  ).multiplyScalar(style === "driven" ? 0.9 : style === "finesse" ? 1.28 : 0.72);
  if (assistedShot) {
  ...
  capBallVelocity(kickVelocity);
  active.cooldown = style === "short" || style === "low-through" ? 0.2 : 0.28;
  const ignoreTimer = player.role === "keeper"
    ? 0.48
    : style === "shot" || style === "driven" || style === "finesse" || style === "chip"
      ? 0.18
      : style === "short"
        ? 0.24
        : style === "long"
          ? 0.28
          : 0.2;
  const touchAction: BallTouchAction = untargetedKick === "clearance"
    ? "clearance"
    : shotStyle
      ? "shot"
      : "pass";
  ...
  active.pendingKickContact = {
    playerId: player.id,
    receiverId: intendedReceiver?.id ?? null,
    style,
    remaining: animationProfile.contactAfter,
    contactPosition: active.ballPos.clone(),
    velocity: kickVelocity,
    curve: kickCurve,
    angularVelocity: kickAngularVelocity,
    grounded: kickGrounded,
    direction: kickDirection.clone(),
    target: resolvedTarget.clone(),
    power: kickPower,
    touchAction,
    ignoreTimer,
    soundVolume: force.power > 26 ? 1.55 : 1.2,
  };
  active.renderer.domElement.dataset.kickContactState = "preparing";
  return true;
}
...
function kickTargetForStyle(player: PlayerBody, active: MatchRuntime, teammate: PlayerBody, style: "short" | "long" | "through" | "low-through") {
  ...
  const receiverProgress = (teammate.pos.z - player.pos.z) * attackSign;
  const advancingTarget = receiverProgress > 2.5;
  ...
  const forwardLead = style === "through" && advancingTarget
    ? 4.8
    : style === "low-through"
      ? advancingTarget ? 4.4 : 0
      : style === "long" && advancingTarget
        ? 4.2
        : 0;
  ...
  if (style === "long" && advancingTarget && (teammate.line === "forward" || Math.abs(teammate.pos.x) > FIELD_W * 0.26)) {
    ...
    const leadDistance = clamp(player.pos.distanceTo(teammate.pos) * 0.075, 2.4, 5.2);
    ...
    target.z = clamp(target.z + attackSign * 1.2, -FIELD_L / 2 + 4, FIELD_L / 2 - 4);
  }
  if ((style === "through" || style === "low-through") && advancingTarget) {
...
function defensiveBuildoutPassRisk(
  player: PlayerBody,
  receiver: PlayerBody,
  target: THREE.Vector3,
  active: MatchRuntime,
  ...
  if (player.role !== "keeper" && player.line !== "defender") {
    return { score: 0, blockers: 0, centralPenalty: 0, arrivalPressure: 0 };
  }
  const blockers = opponentsBetween(player, target, active.players, laneWidth);
  const arrivalPressure = opponentPressureAtPoint(player.team, target, active.players, 5.4);
  const passerPressure = opponentPressure(player, active.players, 6.4);
  const midpointX = (player.pos.x + target.x) * 0.5;
  const centralLane = Math.abs(midpointX) < FIELD_W * 0.14;
  const centralPenalty = centralLane ? 8 + passerPressure * 7 : 0;
  const receiverOpen = nearestOpponentDistance(receiver, active.players);
  const closestOpponent = nearestOpponentTo(receiver, active.players);
  const movingAwayFromPressure = Boolean(
    receiver.vel.lengthSq() > 0.4
    && closestOpponent
    && closestOpponent.pos.clone().sub(receiver.pos).setY(0).dot(receiver.vel.clone().setY(0)) < 0,
  );
  const score = blockers * 32
    + arrivalPressure * 18
    + centralPenalty
    + Math.max(0, 4.8 - receiverOpen) * 5
    - (movingAwayFromPressure ? 4 : 0);
  return { score, blockers, centralPenalty, arrivalPressure };
}
...
function performValidatedAiPass(
  ...
  const buildoutRisk = defensiveBuildoutPassRisk(player, intended, target, active, finalLaneWidth);
  const safeBuildout = buildoutRisk.score < (style === "short" ? 31 : 25);
  if (typeof window !== "undefined" && new URLSearchParams(window.location.search).has("buildoutDebug")) {
    active.renderer.domElement.dataset.buildoutReceiver = intended.id;
    active.renderer.domElement.dataset.buildoutRisk = buildoutRisk.score.toFixed(2);
    active.renderer.domElement.dataset.buildoutCentralPenalty = buildoutRisk.centralPenalty.toFixed(2);
    active.renderer.domElement.dataset.buildoutRejected = String(!safeBuildout || blocked || !arrivalSafe);
  }
  if (!blocked && arrivalSafe && safeBuildout) {
  ...
  const alternatives = active.players
    ...
    .map((candidate) => {
      ...
      const risk = defensiveBuildoutPassRisk(player, candidate, alternativeTarget, active, 3.35);
      const outletBonus = candidate.line === "defender" || candidate.line === "midfielder" || candidate.role === "keeper" ? 5 : 0;
      return { candidate, blockers, risk, score: open * 2.2 + clamp(forward, -12, 18) - distance * 0.18 + arrival * 8 + outletBonus - risk.score };
    })
    .filter(({ candidate, blockers, risk }) => blockers === 0 && risk.score < 28 && nearestOpponentDistance(candidate, active.players) > 4.6)
    .sort((a, b) => b.score - a.score);
...
function performPassTo(player: PlayerBody, active: MatchRuntime, teammate: PlayerBody, style: "short" | "long" | "through" | "low-through", oneTwo = false) {
  ...
  if (passed && !manuallyControlled && active.frameCount % 4 === 0) playCommentary(active, "buildup");
  return passed;
}
...
function performCurvedPassTo(player: PlayerBody, active: MatchRuntime, teammate: PlayerBody) {
  ...
  updatePendingKickTrajectory(active, undefined, curvedPassSpin(player, target, active, active.pendingKickContact?.velocity.length() ?? 0));
...
function performLoftedPassTo(player: PlayerBody, active: MatchRuntime, teammate: PlayerBody) {
  ...
  if (!passed) return false;
  updatePendingKickTrajectory(
    active,
    plan.velocity,
    curvedPassSpin(player, plan.target, active, plan.velocity.length()).multiplyScalar(0.34),
  );
...
function chooseDefensiveBuildupTarget(player: PlayerBody, active: MatchRuntime) {
  ...
  return active.players
    .filter((candidate) => (
      ...
      && !candidate.sentOff
      && (candidate.role !== "keeper" || player.role !== "keeper")
    ))
    .map((candidate) => {
      const target = kickTargetForStyle(player, active, candidate, "short");
      const distance = target.distanceTo(player.pos);
      const laneBlockers = opponentsBetween(player, target, active.players, 3.85);
      const receiverPressure = opponentPressureAtPoint(player.team, target, active.players, candidate.role === "keeper" ? 9.2 : 6.8);
      const open = nearestOpponentDistance(candidate, active.players);
      const forward = (candidate.pos.z - player.pos.z) * attackSign;
      const towardOwnGoal = Math.abs(candidate.pos.z - ownGoalZ) < Math.abs(player.pos.z - ownGoalZ) - 2.5;
      const touchlineRisk = Math.max(0, Math.abs(target.x) - (FIELD_W / 2 - 10));
      const goalLineRisk = Math.max(0, 12 - Math.abs(target.z - ownGoalZ));
      const receiverMovingAwayFromPressure = candidate.vel.lengthSq() > 0.3
        && nearestOpponentTo(candidate, active.players)
        ? candidate.vel.clone().setY(0).dot(
          candidate.pos.clone().sub(nearestOpponentTo(candidate, active.players)?.pos ?? candidate.pos).setY(0),
        ) > 0
        : false;
      const roleValue = candidate.line === "midfielder"
        ? 13
        : candidate.line === "defender"
          ? 8
          : candidate.role === "keeper"
            ? 5
            : 2;
      const score = roleValue
        + clamp(open, 0, 14) * 3
        + clamp(forward, -8, 18) * 0.7
        + (receiverMovingAwayFromPressure ? 4 : 0)
        - laneBlockers * 32
        - receiverPressure * 20
        - touchlineRisk * 3.2
        - goalLineRisk * 1.5
        - (towardOwnGoal ? 8 : 0)
        - Math.abs(distance - 24) * 0.45;
      return {
        candidate,
        score,
        distance,
    ...
    .filter(({ distance, laneBlockers, receiverPressure, open, touchlineRisk, target, candidate }) => (
      distance >= 7.5
      && distance <= 48
      && laneBlockers === 0
      && receiverPressure === 0
      && open >= (candidate.role === "keeper" ? 9 : 5.8)
      && touchlineRisk < 4.5
      && target.distanceTo(candidate.pos) < 4.5
    ))
    .sort((a, b) => b.score - a.score)[0]?.candidate ?? null;
}

function chooseControlledClearanceTarget(player: PlayerBody, active: MatchRuntime) {
  const attackSign = Math.sign(attackingGoalZ(player.team, active.half));
  return active.players
    .filter((candidate) => (
      candidate.team === player.team
    ...
    .map((candidate) => {
      const target = candidate.pos.clone().addScaledVector(candidate.vel, 0.38).setY(BALL_RADIUS);
      const distance = player.pos.distanceTo(target);
      const forward = (target.z - player.pos.z) * attackSign;
      const pressure = opponentPressureAtPoint(player.team, target, active.players, 8.2);
      const open = nearestOpponentDistance(candidate, active.players);
      const laneBlockers = opponentsBetween(player, target, active.players, 5.2);
      return {
        candidate,
        score: forward * 0.72 + open * 2.4 - pressure * 24 - laneBlockers * 5 - Math.abs(distance - 48) * 0.3,
        target,
        distance,
        forward,
        pressure,
    ...
    .filter(({ distance, forward, pressure, open }) => distance > 28 && distance < 68 && forward > 8 && pressure === 0 && open > 6.5)
    .sort((a, b) => b.score - a.score)[0] ?? null;
}
  ...
  if (style === "short") {
    return candidates
      ...
      .map((teammate) => {
        ...
        const buildoutRisk = defensiveBuildoutPassRisk(player, teammate, target, active, 2.9);
        ...
        return {
          teammate,
          score: distanceScore + directionScore + facingScore + receiverRunScore + clamp(open, 0, 12) * 2.05 - laneBlockers * 6.2 - buildoutRisk.score - (riskyBackPass ? 48 : 0),
          distance,
          laneBlockers,
          open,
          riskyBackPass,
      ...
      ?? candidates
        ...
        .filter((teammate) => {
          ...
          const open = nearestOpponentDistance(teammate, active.players);
          const risk = defensiveBuildoutPassRisk(player, teammate, target, active, 3.2);
          return risk.score < 31 && shortPassLaneIsUsable(player, teammate, active, target, blockers, open) && !isRiskyBackPass(player, teammate, active, blockers, open);
        })
        .sort((a, b) => {
...
function attemptTackle(player: PlayerBody, active: MatchRuntime) {
  ...
  const aiReach = ownerSlow
    ? DEFENSIVE_INTERCEPTION_RANGE * (0.56 + DEFENSIVE_TACKLE_AGGRESSION * 0.05)
    : DEFENSIVE_INTERCEPTION_RANGE * (0.39 + DEFENSIVE_TACKLE_AGGRESSION * 0.04);
  if (ballDistance > aiReach + manualReach + exposedReach || ballDistance <= 0.05) return false;
  ...
  releasePossession(active, "loose");
  active.ballPos.copy(ballPoint).setY(Math.max(BALL_RADIUS, active.ballPos.y));
  const tackleContact: FirstTouchContact = {
    distance: ballDistance,
    point: active.ballPos.clone(),
    type: "foot",
  };
  if (!takePossession(player, active, tackleContact)) return false;
