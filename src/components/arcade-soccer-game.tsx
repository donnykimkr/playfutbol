...
import { GraduationCap, Play, RotateCcw, SkipForward, X } from "lucide-react";
import {
...
} from "@/components/futbahl-match-ui";
import {
  ACTION_COOLDOWN,
  ...
  AD_BOARD_INNER_X,
  AD_BOARD_INNER_Z,
  ADAPTIVE_TOUCHLINE_CAMERA,
  AWAY_COLOR,
  AWAY_KEEPER_COLOR,
  AWAY_SHORTS,
  AWAY_TRIM,
  BALL_AIR_DRAG,
  BALL_ENTER_GROUNDED_THRESHOLD,
  BALL_GRAVITY,
  BALL_MASS_KG,
  BALL_MAX_SPEED,
  BALL_MAX_SUBSTEPS,
  ...
  BROADCAST_CAMERA_Y,
  BROADCAST_CAMERA_Z,
  BROADCAST_CAMERA_Z_OFFSET,
  BROADCAST_LOOK_AT_X,
  BROADCAST_LOOK_AT_Y,
  BROADCAST_LOOK_AT_Z,
  CAMERA_FOLLOW_DAMPING,
  CAMERA_TOUCHLINE_DAMPING,
  CLOCK_SPEED,
  CONTROL_TOUCH_DISTANCE,
  CROWD_AMBIENCE_CROSSFADE_SECONDS,
  CROWD_AMBIENCE_LAYER_GAIN,
  CROWD_GOAL_COOLDOWN_MS,
  CROWD_MASTER_GAIN,
  CROWD_SWELL_COOLDOWN_MS,
  DEFENSIVE_INTERCEPTION_RANGE,
  DEFENSIVE_MARKING_DISTANCE,
  DEFENSIVE_PRESSING_DISTANCE,
  DEFENSIVE_REACTION_DELAY,
  DEFENSIVE_RECOVERY_SPEED,
  DEFENSIVE_TACKLE_AGGRESSION,
  DRIBBLE_DISTANCE,
  FIELD_L,
  FIELD_PLAYER_BALL_RADIUS,
  FIELD_W,
  FLOOR_LAYER_MIN_SEPARATION,
  FLOOR_LAYER_NAMES,
  FORMATION_SCALE,
  FULL_TIME_SECONDS,
  GOAL_BACK_Z,
  ...
  GOAL_FRONT_Z,
  GOAL_H,
  GOAL_KICK_CURSOR_SPEED,
  GOAL_POST_CENTER_X,
  GOAL_POST_THICKNESS,
  GOAL_SCORE_Z,
  GOAL_SIDE_POST_INSET,
  GOAL_W,
  HALF_TIME_SECONDS,
  ...
  PLAYER_NORMAL_SPEED,
  PLAYER_RADIUS,
  RECORDED_CROWD_AMBIENT_PATH,
  RECORDED_CROWD_GOAL_PATH,
  RECORDED_CROWD_SWELL_PATH,
  RECORDED_REFEREE_WHISTLE_PATH,
  SHARED_BALL_PHYSICS,
  SHOT_INPUT_LOCK_SECONDS,
  SHOT_VERTICAL_POWER_MULTIPLIER,
  SPRINT_SPEED,
  STAMINA_DRAIN_RATE,
  ...
  cameraSideTouchlineBlend,
  fitDirectionalShadowCameraToPitch,
  projectedViewportY,
...
import type {
  ...
  BallTouchBodyPart,
  BallTouchRecord,
  BoundaryRestartPhase,
  DefensiveDangerPhase,
  DefensivePressureRole,
  DefensiveTacticalRole,
  DefensiveTeamPlan,
  FirstTouchType,
  ...
  TutorialStatus,
  WhistleEvent,
} from "@/lib/futbahl-match-types";
import {
  finishAnalyticsSession,
  getAnonymousVisitorId,
  startAnalyticsSession,
  trackVisitorPageView,
} from "@/lib/futbahl-analytics";
import {
  DEFAULT_MATCH_SETTINGS,
  detectAppleMobile,
  fixedFormationSettings,
  loadOfflineSettings,
  saveOfflineSettings,
  type OfflineSettings,
} from "@/lib/futbahl-offline-settings";
import {
  addGoal,
  addLightweightStadium,
  addPitch,
  createBlobShadow,
  createKickTrajectoryPreview,
  createPassTargetMarker,
  createSoccerBall,
  resolveAdvertisingBoardCollision,
  resolveGoalFrameCollision,
  updateAdvertisingBoards,
} from "@/lib/futbahl-match-scene";
import {
  disposeObjectTree,
  disposeSharedResources,
} from "@/lib/futbahl-three-resources";
import { makeKit } from "@/lib/futbahl-player-model";
import {
  attackingGoalZ,
  cornerKickDirection,
  forwardFromHeading,
  goalKickBallSpot,
  goalKickBallSpotForTeam,
  goalKickKeeperSpot,
  goalKickRunupSpot,
  headingForHome,
  headingFromDirection,
  opponent,
  pointInsideOwnPenaltyArea,
  teamGoalZ,
  teamSide,
  throwInTakerSpot,
  upfieldKickDirection,
} from "@/lib/futbahl-pitch-geometry";
import {
...
let activeOfflineSettings: OfflineSettings = fixedFormationSettings(DEFAULT_MATCH_SETTINGS);
...
function createPlayer(id: string, team: TeamId, role: PlayerRole, line: PlayerLine, x: number, z: number, number: number, formationSlot: string, controlledBy?: "p1") {
  ...
  const mesh = makeKit(team, role, "#ffffff", bodyPresetId, activeOfflineSettings.homeColor);
...
export function ArcadeSoccerGame() {
  ...
  const [offlineSettings, setOfflineSettings] = useState<OfflineSettings>(() => fixedFormationSettings(DEFAULT_MATCH_SETTINGS));
  const [draftSettings, setDraftSettings] = useState<OfflineSettings>(() => fixedFormationSettings(DEFAULT_MATCH_SETTINGS));
  ...
  return (
    ...
    >
      ...
      {matchState === "playing" && !tutorialUi.active && (
        <MatchScoreboard
          homeName={offlineSettings.userTeam.name}
          homeColor={offlineSettings.homeColor}
          awayName={offlineSettings.aiTeam.name}
          awayColor={AWAY_COLOR}
          score={score}
          gameClock={gameClock}
          goalBannerTeam={goalBannerTeam}
        />
      )}
      {matchState === "playing" && !tutorialUi.active && !showTouchControls && (
        <DesktopMatchControls
          aiEnabled={p1AiUi}
          guideOpen={keyboardGuideOpen}
          appleMobile={isAppleMobile}
          onOpenSettings={openSettings}
          onToggleGuide={() => setKeyboardGuideOpen((open) => !open)}
          onToggleAi={() => {
            const active = sceneRef.current;
            if (!active || active.state !== "playing") return;
            setP1AutopilotMode(active, !active.p1Autopilot);
      ...
      {matchState === "playing" && shotGaugeVisible && (
        <ShotPowerGauge charge={shotChargeUi} x={shotChargePosition.x} y={shotChargePosition.y} />
      )}
      {matchState === "playing" && staminaGaugeVisible && (
        <StaminaGauge stamina={staminaUi} x={staminaPosition.x} y={staminaPosition.y} />
      )}
      {matchState === "playing" && !tutorialUi.active && offlineSettings.minimapEnabled && minimapSnapshot && (
        <MatchMinimap snapshot={minimapSnapshot} />
      )}
      ...
      {settingsOpen && (
        <MatchSettingsDialog
          cameraMode={cameraModeUi}
          draftSettings={draftSettings}
          matchInProgress={matchState === "playing"}
          ...
          onHomeColor={(homeColor) => setDraftSettings((current) => ({ ...current, homeColor }))}
          onMinimap={(enabled) => {
          ...
          onCrowdVolume={(crowdVolume) => setDraftSettings((current) => ({ ...current, crowdVolume }))}
          onExitGame={() => {
            closeSettings();
            endMatch();
          }}
